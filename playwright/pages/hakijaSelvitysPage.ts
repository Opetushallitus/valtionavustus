...
const outcomeLocator = (page: Page, radioNumber: number) =>
  page.locator(
    `label[for='project-outcomes.project-outcomes-1.outcome-type.radio.${radioNumber}']`
  );
...
export const HakijaSelvitysPage = (page: Page) => ({
  ...
  outcomeDescription: page.locator(
    `[name='project-outcomes.project-outcomes-1.description']`
  ),
  outcomeAddress: page.locator(
  ...
  valiselvitysWarning: page.locator(
    'text="Väliselvityksen jättämisen määräaika on umpeutunut"'
  ),
