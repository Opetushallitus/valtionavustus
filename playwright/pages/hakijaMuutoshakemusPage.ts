...
export class HakijaMuutoshakemusPage {
  ...
  async fillTalousarvioValues(budget: Partial<BudgetAmount>, explanation?: string) {
    ...
    if (explanation) {
      await this.locators().budget.input.taloudenKayttosuunnitelmanPerustelut.fill(explanation)
    }
  }
  ...
  expectApprovedBudgetToBe(page: Page, budget: BudgetAmount): () => Promise<void> {
    return async function verifyBudget() {
      ...
      expect(sortedFormTable(budgetRows)).toEqual(sortedFormTable(budgetExpectedItems))
    }
  }
  ...
  async sendMuutoshakemus(isApplication: boolean, swedish?: boolean) {
    if (swedish) {
      ...
      expect(await this.page.textContent('div[class="auto-hide success"]')).toEqual(
        'Ändringsansökan har skickats'
      )
    } else {
  ...
  locators() {
    const originalHakemusIframe = this.page.frameLocator('iframe[data-test-id="original-hakemus"]')
    return {
      avustushakuName: this.page.locator('[data-test-id=avustushaku-name]'),
      projectName: this.page.locator('[data-test-id=project-name]'),
      registerNumber: this.page.locator('[data-test-id="register-number-title"]'),
      contactPerson: this.page.locator('#muutoshakemus__contact-person'),
      contactPersonLabel: this.page.locator('label[for="muutoshakemus__contact-person"]'),
      ...
      existingJatkoaikaTitle: this.page.locator('[data-test-id="jatkoaika-title-existing"]'),
      newJatkoaikaTitle: this.page.locator('[data-test-id="jatkoaika-title-new"]'),
      ...
      budget: {
        ...
        input: {
          personnelCosts: this.page.locator('input[name="talousarvio.personnel-costs-row"]'),
          equipmentCosts: this.page.locator('input[name="talousarvio.equipment-costs-row"]'),
          materialCosts: this.page.locator('input[name="talousarvio.material-costs-row"]'),
        ...
        expensesTotalTitle: this.page.locator('[data-test-id="expenses-total-title"]'),
        taloudenKayttosuunnitelmanPerustelutLabel: this.page.locator(
          'label[for="perustelut-taloudenKayttosuunnitelmanPerustelut"]'
        ),
        budgetRows: this.page.locator('[data-test-id=meno-input-row]').locator('.description'),
