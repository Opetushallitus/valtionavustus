...
export const KoodienhallintaPage = (page: Page) => {
  ...
  const codeRowLocator = (year: string, name: string, code: string) => page.locator(`tr[data-test-id="code-cell-${year}-${code}-${name}"]`)
  const firstCellLocator = (year: string, name: string, code: string) => codeRowLocator(year, name, code).locator('td:first-of-type')
  ...
  return {
    ...
    clickCodeVisibilityButton: async (year: string, name: string, code: string, visibility: boolean) => {
      const buttonId = visibility ? 'show-code' : 'hide-code'
      await codeRowLocator(year, name, code).locator(`[data-test-id=${buttonId}]`).click()
      await page.waitForLoadState('networkidle')
    },
    createCode,
    assertCodeIsVisible: async (year: string, name: string, code: string) => {
      await expect(firstCellLocator(year, name, code)).not.toHaveClass('code-cell__hidden')
    },
    assertCodeIsHidden: async (year: string, name: string, code: string) => {
      await expect(firstCellLocator(year, name, code)).toHaveClass('code-cell__hidden')
    },
    createCodeValues,
