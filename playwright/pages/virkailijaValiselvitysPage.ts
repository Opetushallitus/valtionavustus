...
function SelvitysTab(page: Page, type: SelvitysType) {
  ...
  async function acceptSelvitys() {
    ...
    await page.waitForSelector(
      `text=Lähetetty ${umlautType}selvityksen hyväksyntä`
    );
  }
...
export function VirkailijaValiselvitysPage(page: Page) {
  async function navigateToValiselvitysTab(
  ...
  async function navigateToSelvitysTab(
    ...
    await navigate(
      page,
      `/avustushaku/${avustushakuID}/hakemus/${hakemusID}/${type}selvitys/`
    );
    await page.waitForSelector(
      `[data-test-id="hakemus-details-${type}selvitys"]`,
      { state: "visible" }
    );
