...
export class HakijaAvustusHakuPage {
  ...
  async fillKoulutusosioHakemus(
    ...
    await this.page
      ...
      .first()
      .fill('Esmo Esimerkki')
    await this.page.locator(`label:has-text("Yhteyshenkilön nimi")`).first().fill('Esmo Esimerkki')
    await this.page
    ...
    await this.page.getByLabel('Miten osallistujat rekrytoidaan koulutukseen?').fill('inspiroidaan')
    await this.page
    ...
    await this.page.locator("[name='namedAttachment-0']").setInputFiles(dummyExcelPath)
    await this.page.locator(`[for="vat-included.radio.1"]`).click()
    await this.page.locator(`[id="personnel-costs-row.amount"]`).fill('1000')
    await this.page.locator(`[id="own-income-row.description"]`).fill('Oma osuus')
