...
const test = selvitysTest.extend<{
  ...
  secondHakuProps: async (
    { valiAndLoppuselvitysSubmitted, page, hakuProps },
    use
  ) => {
    expectToBeDefined(valiAndLoppuselvitysSubmitted);
    const hakujenHallintaPage = new HakujenHallintaPage(page);
    const secondAvustushakuName = hakuProps.avustushakuName.replace(
      prefix1,
      prefix2
    );
    ...
    const avustushakuID =
      await hakujenHallintaPage.createMuutoshakemusEnabledHaku(secondHakuProps);
...
test("sorting haku table", async ({
  ...
  const secondAvustushakuStartFormatted =
    moment(hakuaikaStart).format("DD.MM.YY");
  ...
  await test.step("correct tila for avustushaut", async () => {
    ...
    await expect(tila.cellValue(secondAvustushakuName)).toContainText(
      "Julkaistu"
    );
  });
  ...
  await test.step("correct päätös for avustushaut", async () => {
    await expect(paatos.cellValue(avustushakuName)).toContainText(
      todayFormatted
    );
  ...
  await test.step("correct maksatukset for avustushaut", async () => {
    await expect(maksatukset.cellValue(avustushakuName)).toContainText(
      todayFormatted
    );
  ...
  await test.step("correct valiselvitykset for avustushaut", async () => {
    await expect(valiselvitykset.cellValue(avustushakuName)).toContainText(
      todayFormatted
    );
    await expect(
      valiselvitykset.cellValue(secondAvustushakuName)
    ).toContainText("-");
  });
  ...
  await test.step("correct loppuselvitykset for avustushaut", async () => {
    await expect(loppuselvitykset.cellValue(avustushakuName)).toContainText(
      todayFormatted
    );
    await expect(
      loppuselvitykset.cellValue(secondAvustushakuName)
    ).toContainText("-");
  });
  ...
  await test.step("correct valmistelija for avustushaut", async () => {
    await expect(vastuuvalmistelija.cellValue(avustushakuName)).toContainText(
      "_v"
    );
    await expect(
      vastuuvalmistelija.cellValue(secondAvustushakuName)
    ).toContainText("_v");
  });
  ...
  await test.step("correct muutoshakukelpoinen for avustushaut", async () => {
    await expect(muutoshakukelpoinen.cellValue(avustushakuName)).toContainText(
      "Kyllä"
    );
    await expect(
      muutoshakukelpoinen.cellValue(secondAvustushakuName)
    ).toContainText("Ei");
  });
  ...
  await test.step("correct kayttoaika alkaa for avustushaut", async () => {
    await expect(kayttoaikaAlkaa.cellValue(avustushakuName)).toContainText(
      "20.04.69"
    );
    await expect(
      kayttoaikaAlkaa.cellValue(secondAvustushakuName)
    ).toContainText(secondHakuProps.hankkeenAlkamispaiva);
  });
  ...
  await test.step("correct kayttoaika paattyy for avustushaut", async () => {
    await expect(kayttoaikaPaattyy.cellValue(avustushakuName)).toContainText(
      "20.04.00"
    );
    await expect(
      kayttoaikaPaattyy.cellValue(secondAvustushakuName)
    ).toContainText(secondHakuProps.hankkeenPaattymispaiva);
  });
  ...
  await test.step("correct jaossa ollut summa for avustushaut", async () => {
    await expect(jaossaOllutSumma.cellValue(avustushakuName)).toContainText(
      "100000"
    );
    await expect(
      jaossaOllutSumma.cellValue(secondAvustushakuName)
    ).toContainText("-");
  });
  await test.step("can be sorted by jaossa ollut summa", async () => {
    ...
    await test.step("correct maksettu summa for avustushaut", async () => {
      await expect(maksettuSumma.cellValue(avustushakuName)).toContainText(
        "99999"
      );
      await expect(
        maksettuSumma.cellValue(secondAvustushakuName)
      ).toContainText("-");
    });
