...
muutoshakemusTest.extend<
  Pick<MuutoshakemusFixtures, "finalAvustushakuEndDate">
>({
  ...
  async ({ page, hakuProps, closedAvustushaku }) => {
    ...
    const email = await getLastAvustushakuEmail(
      closedAvustushaku.id,
      "hakuaika-paattynyt"
    );
    expect(email).toBeDefined();

    await test.step("notification is not sent to arvioija", async () => {
      expect(email["to-address"]).not.toContain(
        "paivi.paakayttaja@example.com"
      );
    });
...
>({
  ...
  async ({ page, closedAvustushaku }) => {
    await sendHakuaikaPaattynytNotifications(page);
    const emails = await getAvustushakuEmails(
      closedAvustushaku.id,
      "hakuaika-paattynyt"
    );
...
const sendHakuaikaPaattynytNotifications = (page: Page) =>
  page.request.post(
    `${VIRKAILIJA_URL}/api/test/send-hakuaika-paattynyt-notifications`,
