...
test.describe("loppuselvitys-palauttamatta", () => {
  ...
  loppuselvitysTest(
    ...
    async ({
      ...
      expectToBeDefined(loppuselvityspyyntöSent);
      const loppuselvitysdate = moment()
        .subtract(1, "days")
        .format("DD.MM.YYYY");
      await setLoppuselvitysDate(page, avustushakuID, loppuselvitysdate);
    ...
    async ({
      ...
      await test.step("reminder is sent", async () => {
        ...
        expect(email.subject).toContain(
          "Muistutus loppuselvityksen palauttamisesta"
        );
...
Lisätietoja saatte tarvittaessa avustuspäätöksessä mainitulta lisätietojen antajalta. Teknisissä ongelmissa auttaa: valtionavustukset@oph.fi`);
      ...
      await test.step("reminder is not sent again", async () => {
        await sendLoppuselvitysPalauttamattaNotifications(page);
        const emailsAfterSecondSend = await getLoppuselvitysPalauttamattaEmails(
          hakemusID
        );
  ...
  loppuselvitysTest.extend<{ answers: Answers }>({
    ...
    async ({
      ...
      const email = lastOrFail(
        await getLoppuselvitysPalauttamattaEmails(hakemusID)
      );
      expect(email["to-address"]).toHaveLength(1);
      expect(email["to-address"]).toContain("lars.andersson@example.com");
      expect(email.subject).toContain(
        "Påminnelse om att lämna in slutredovisningen"
      );
  ...
  loppuselvitysTest(
    ...
    async ({
      ...
      await test.step("check reminder email is not sent", async () => {
        ...
        await sendLoppuselvitysPalauttamattaNotifications(page);
        const emailsAfter = await getLoppuselvitysPalauttamattaEmails(
          hakemusID
        );
        expect(emailsAfter).toEqual(emailsBefore);
      });
...
const sendLoppuselvitysPalauttamattaNotifications = (page: Page) =>
  page.request.post(
    `${VIRKAILIJA_URL}/api/test/send-loppuselvitys-palauttamatta-notifications`,
    { failOnStatusCode: true }
  );

async function setLoppuselvitysDate(
  page: Page,
