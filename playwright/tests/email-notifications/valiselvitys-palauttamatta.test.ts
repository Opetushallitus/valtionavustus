...
test.describe("valiselvitys-palauttamatta", () => {
  väliselvitysTest('reminder in Finnish', async ({page, hakuProps, avustushakuID, acceptedHakemus: { hakemusID, userKey }, väliselvityspyyntöSent}) => {
...
Lisätietoja saatte tarvittaessa avustuspäätöksessä mainitulta lisätietojen antajalta. Teknisissä ongelmissa auttaa: valtionavustukset@oph.fi`)
  ...
  })('reminder mail is sent in swedish for swedish hakemus', async ({page, hakuProps, avustushakuID, acceptedHakemus: { hakemusID, userKey }, väliselvityspyyntöSent}) => {
    expectToBeDefined(väliselvityspyyntöSent)
    const valiselvitysdate = moment().add(14, 'days').format('DD.MM.YYYY')
    await setValiselvitysDate(page, avustushakuID, valiselvitysdate)
...
er mellanredovisning för användningen av statsunderstödet ${hakuProps.avustushakuName + ' på svenska'} har ännu inte lämnats in.
...
`)
  ...
  väliselvitysTest('do not send reminders if valiselvitys pyyntö has not been sent', async ({ page, avustushakuID, acceptedHakemus: { hakemusID }}) => {
...
const sendValiselvitysPalauttamattaNotifications = (page: Page) =>
  page.request.post(`${VIRKAILIJA_URL}/api/test/send-valiselvitys-palauttamatta-notifications`, { failOnStatusCode: true })
