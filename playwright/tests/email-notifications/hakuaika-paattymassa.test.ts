...
const sendHakuaikaPaattymassaNotifications = (page: Page) =>
  page.request.post(
    `${VIRKAILIJA_URL}/api/test/send-hakuaika-paattymassa-notifications`
  );
...
const hakuaikaPaattymassaTest =
  muutoshakemusTest.extend<HakuaikaPaattymassaFixtures>({
    filledHakemus: async (
      { avustushakuID, browser, answers },
      use,
      testInfo
    ) => {
    ...
    swedishHakemus: async (
      { avustushakuID, page, swedishAnswers },
      use,
      testInfo
    ) => {
      testInfo.setTimeout(testInfo.timeout + 30_000);
