...
test('loppuselvitys-asiatarkastamatta notification is not sent to virkailija if already asiatarkastettu', async ({
  ...
  const oldEmails = await getLoppuselvitysAsiatarkastamattaEmails(avustushakuID)
  ...
  const allEmails = await getLoppuselvitysAsiatarkastamattaEmails(avustushakuID)
