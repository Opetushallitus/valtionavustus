...
test.describe("Koodienhallinta", () => {
  ...
  test("koodienhallinta tab is hidden from non admin user", async ({
    page,
  }) => {
    await navigate(page, "/");
  ...
  test("koodienhallinta tab can not be navigated to by non admin user", async ({
    page,
  }) => {
    ...
    const responseForNonAdmin = await navigate(
      page,
  ...
  test("validates year input", async ({ koodienhallintaPage }) => {
    ...
    await koodienhallintaPage.codeInput.fill("testCode");
    await koodienhallintaPage.submitButton.isDisabled();
    await koodienhallintaPage.codeInputFormHasError(
  ...
  test("navigating between tabs clears values in form", async ({
    koodienhallintaPage,
  }) => {
    await koodienhallintaPage.yearInput.fill("2022");
  ...
  test("When a code is created", async ({ koodienhallintaPage }) => {
    ...
    const codeYear = "2020";
    const rowLocator = koodienhallintaPage.codeRowLocator(
      codeYear,
      codeName,
    ...
    await test.step("And virkailija hides the code", async () => {
      ...
      await test.step("the code is not visible", async () => {
        await expect(rowLocator).toBeVisible();
        await koodienhallintaPage.assertCodeIsHidden(
          codeYear,
          codeName,
          codeValues.operationalUnit
        );
