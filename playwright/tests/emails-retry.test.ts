...
async function deleteEmailsAndEmailEvents(request: APIRequestContext) {
  await request.post(`${VIRKAILIJA_URL}/api/test/email/delete-generated-emails-and-events`, {failOnStatusCode: true})
}
...
async function generateEmailsThatFailedToBeSent(request: APIRequestContext, count: number) {
  await request
    .post(`${VIRKAILIJA_URL}/api/test/email/generate-emails-that-failed-to-be-sent`, {
      data: {count}
    })
}

async function retryToSendEmail(request: APIRequestContext) {
  await request.post(`${VIRKAILIJA_URL}/api/test/email/retry-to-send-email`, {failOnStatusCode: true})
}
