...
async function deleteEmailsAndEmailEvents(request: APIRequestContext) {
  await request.post(
    `${VIRKAILIJA_URL}/api/test/email/delete-generated-emails-and-events`,
...
async function getEmailsThatSucceededToBeSent(request: APIRequestContext) {
  const res = await request.get(
    `${VIRKAILIJA_URL}/api/test/email/sent/succeeded`
  );
  return getGeneratedEmails(await res.json());
}

async function generateEmailsThatFailedToBeSent(
  request: APIRequestContext,
  count: number
) {
  await request.post(
    `${VIRKAILIJA_URL}/api/test/email/generate-emails-that-failed-to-be-sent`,
    {
      data: { count },
    }
  );
