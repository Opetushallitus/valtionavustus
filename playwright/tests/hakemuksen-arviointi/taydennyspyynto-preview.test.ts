...
test("Virkailija can preview täydennyspyyntö", async ({
  ...
  expect(
    await page.textContent("[data-test-id='change-request-preview-subject']")
  ).toStrictEqual(`Otsikko: Täydennyspyyntö avustushakemukseesi`);
  expect(
    await page.textContent("[data-test-id='change-request-preview-sender']")
  ).toStrictEqual(`Lähettäjä: no-reply@csc.fi`);
  expect(
    await page.textContent("[data-test-id='change-request-preview-content']")
