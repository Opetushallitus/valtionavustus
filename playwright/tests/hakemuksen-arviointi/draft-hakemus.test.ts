...
const newHakemusTest = muutoshakemusTest.extend<NewHakemusFixtures>({
  newHakemus: async ({ avustushakuID, page }, use, testInfo) => {
    ...
    await test.step("Create new hakemus link", async () => {
      ...
      hakemusUrl = await hakijaAvustusHakuPage.startApplication(
        avustushakuID,
        email
      );
    });
...
newHakemusTest(
  ...
  async ({ avustushakuID, newHakemus, page }) => {
    ...
    await test.step("can be submitted by admin", async () => {
      ...
      expect(
        await hakemusListPage.hakemusRows
          .first()
          .locator("text=Käsittelemättä")
          .isVisible()
      ).toBeTruthy();
      await expect(page.locator('[data-test-id="submit-hakemus"]')).toHaveCount(
        0
      );
    });
  }
