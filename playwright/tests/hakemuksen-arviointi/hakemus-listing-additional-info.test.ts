...
test(`hakemusten arviointi additional info`, async ({ page, avustushakuID, closedAvustushaku, answers, codes}) => {
  expectToBeDefined(closedAvustushaku)
  const hakemustenArviointiPage = new HakemustenArviointiPage(page)
  await hakemustenArviointiPage.navigate(avustushakuID, {newListingUi: true})
  await hakemustenArviointiPage.acceptAvustushaku(avustushakuID, answers.projectName)
  ...
  await test.step('updates paatos to show when it was sent', async () => {
    ...
    await hakemustenArviointiPage.page.locator('[aria-label="Lisää valmistelija hakemukselle"]').click()
    ...
    await hakemustenArviointiPage.navigate(avustushakuID, {newListingUi: true, showAdditionalInfo: true})
