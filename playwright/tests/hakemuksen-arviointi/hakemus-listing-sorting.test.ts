...
const test = budjettimuutoshakemusTest.extend<ArviointiUiFilteringFixtures>({
  hakemustenArviointiPage: async ({submittedHakemus, page, avustushakuID, answers}, use) => {
    ...
    await hakijaAvustusHakuPage.fillBudjettimuutoshakemusEnabledHakemus(avustushakuID, answers3, budget3)
    ...
    await hakemustenArviointiPage.navigateToHakemus(avustushakuID, submittedHakemus.userKey, { newListingUi: true })
    await hakemustenArviointiPage.acceptHakemus('7000')

    await hakemustenArviointiPage.navigateToHakemus(avustushakuID, userKey2, { newListingUi: true })
    await hakemustenArviointiPage.acceptHakemus('3000')

    await hakemustenArviointiPage.navigateToHakemus(avustushakuID, userKey3, { newListingUi: true })
...
test('hakemus list sorting when avustushaku is not resolved', async ({ hakemustenArviointiPage, hakuProps: { registerNumber }, avustushakuID }) => {
  await hakemustenArviointiPage.navigate(avustushakuID, { newListingUi: true })
  await hakemustenArviointiPage.page.click('[aria-label="Poista hakemuksen tila rajaukset"]')
