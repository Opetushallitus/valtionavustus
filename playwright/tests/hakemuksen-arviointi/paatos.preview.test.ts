...
const lisatekstiAmmatillinenKoulutus =
  "myönteinenlisäteksti ammatillinen koulutus";
...
test("paatos lisäteksti", async ({
  ...
  await test.step(
    "if no talousarviotili selected show default text",
    async () => {
  ...
  await test.step(
    "if koulutusaste selected and have text for koulutusaste show it",
    async () => {
      ...
      await openLuonnosAndExpectToContainText(
        page,
        context,
        lisatekstiAmmatillinenKoulutus
      );
  ...
  await test.step(
    "if koulutusaste selected with no own lisäteksti show default text",
    async () => {
      await taTili.input.click();
