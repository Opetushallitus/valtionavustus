...
selvitysTest(
  ...
  async ({ page, väliselvityspyyntöSent, loppuselvityspyyntöSent, avustushakuID }) => {
    ...
    await expect(firstHakemus.locator(`[data-test-class=valiselvitys-status-cell]`)).toHaveText(
      'Puuttuu'
    )
    await expect(firstHakemus.locator(`[data-test-class=loppuselvitys-status-cell]`)).toHaveText(
      'Puuttuu'
    )
  }
)
