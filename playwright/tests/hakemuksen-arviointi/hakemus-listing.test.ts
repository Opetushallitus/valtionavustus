...
test(`shows Muokattu pill when the application has been modified after submit`, async ({ page, avustushakuID, submittedHakemus}) => {
  ...
  const firstRow = await hakemustenArviointiPage.hakemusRows.first()
  const modifiedPill = await firstRow.locator('[data-test-id$=modified-pill]')
