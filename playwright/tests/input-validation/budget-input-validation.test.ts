...
test('When hakemus is approved with non-numeric budget values', async ({
  ...
  await test.step('Non-numeric characters were ignored for approved budget', async () => {
    const myonnettyLocators = hakemustenArviointiPage.getNormalizedBudget().myonnetty
    for (const [key, value] of Object.entries(budgetWithNumericValues.amount)) {
  ...
  await test.step('Input field does not have error when value is 0', async () => {
    const steamshipInputSelector = '[id="budget-edit-steamship-costs-row"] .soresu-money-field'

    expect(budgetWithNumericValues.amount.steamship).toBe('0')
    await expect(page.locator(steamshipInputSelector)).not.toHaveClass(/error/)
  })
})
