...
const test = muutoshakemusTest.extend<{yhteenveto: {hakemusIds: number[], userKey: string}}>({
  submittedHakemus: async ({avustushakuID, answers, page}, use, testInfo) => {
    testInfo.setTimeout(testInfo.timeout + 30_000)

    const hakijaAvustusHakuPage = new HakijaAvustusHakuPage(page)
    await hakijaAvustusHakuPage.navigate(avustushakuID, answers.lang)
    await hakijaAvustusHakuPage.fillAndSendMuutoshakemusEnabledHakemus(avustushakuID, pyorijatAnswers)

    await hakijaAvustusHakuPage.navigate(avustushakuID, answers.lang)
    const {userKey} = await hakijaAvustusHakuPage.fillAndSendMuutoshakemusEnabledHakemus(avustushakuID, answers)
    await use({userKey})
  },
  yhteenveto: async ({closedAvustushaku, page, ukotettuValmistelija, submittedHakemus: {userKey}, answers}, use, testInfo) => {
    const avustushakuID = closedAvustushaku.id
    testInfo.setTimeout(testInfo.timeout + 50_000)

    const hakemustenArviointiPage = new HakemustenArviointiPage(page)
    await hakemustenArviointiPage.navigate(avustushakuID)
    const hakemusID1 = await hakemustenArviointiPage.acceptAvustushaku(avustushakuID, pyorijatAnswers.projectName, "10000")
    await hakemustenArviointiPage.navigate(avustushakuID)
    const hakemusID2 = await hakemustenArviointiPage.acceptAvustushaku(avustushakuID, answers.projectName, "100000")
...
test('Yhteenveto', async ({page, avustushakuID, hakuProps, yhteenveto: {hakemusIds}, answers}) => {
...
`)
  ...
  await test.step('heading summary', async () => {
    await test.step('accepted summary is correct', async () => {
      const {title, amount, appliedAmount, grantedAmount} = await yhteenvetoPage.summaryHeadingRowFor('accepted')
    ...
    await test.step('combined summary is correct', async () => {
      const {title, amount, appliedAmount, grantedAmount} = await yhteenvetoPage.summaryHeadingRowFor('combined')
  ...
  await test.step('accepted summary table', async () => {
    ...
    await test.step('summary is correct', async () => {
      const {totalAppliedAmount, totalGrantedAmount} = await yhteenvetoPage.summaryTableSummaryRow('accepted')
