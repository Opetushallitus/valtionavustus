...
test('Hakija views muutoshakemus page', async ({hakijaMuutoshakemusPaatosPage}) => {
  ...
  await test.step('budget decision is shown', async () => {
    expect(await hakijaMuutoshakemusPaatosPage.talousarvioPaatos()).toEqual('Hyväksytään haetut muutokset budjettiin muutettuna')
  })
  ...
  await test.step('Approved budget title is shown in finnish', async () => {
    expect(await hakijaMuutoshakemusPaatosPage.currentBudgetHeader()).toEqual('Hyväksytty uusi budjetti')
  })

  await test.step('Jatkoaika decision is shown', async () => {
    expect(await hakijaMuutoshakemusPaatosPage.jatkoaikaPaatos()).toEqual('Hyväksytään haetut muutokset käyttöaikaan muutettuna')
  })

  await test.step('Päätöksen perustelut title is shown in finnish', async () => {
    expect(await hakijaMuutoshakemusPaatosPage.paatoksetPerustelutTitle()).toEqual('Päätöksen perustelut')
  })

  await test.step('päätöksen perustelut is shown', async () => {
    expect(await hakijaMuutoshakemusPaatosPage.paatoksenPerustelut()).toEqual('Opetushallitus on arvioinut hakemuksen. Opetushallitus on asiantuntija-arvioinnin perusteella ja asiaa harkittuaan päättänyt hyväksyä haetut muutokset muutettuna, siten kuin ne kuvataan tässä avustuspäätöksessä.')
  })
  ...
  await test.step('budget change is mentioned in the info section', async () => {
    expect(await hakijaMuutoshakemusPaatosPage.infoSection()).toEqual('Muutoshakemus talouden käyttösuunnitelmaan.')
  })
