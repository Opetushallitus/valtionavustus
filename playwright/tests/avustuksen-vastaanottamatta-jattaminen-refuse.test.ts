...
muutoshakemusTest('Avustuksesta vastaanottamatta jättäminen', async ({ page, avustushakuID, acceptedHakemus }) => {
  ...
  await muutoshakemusTest.step('Hakemus list shows refused application', async () => {
    const hakemustenArviointiPage = new HakemustenArviointiPage(page)
    await hakemustenArviointiPage.navigate(avustushakuID, { newListingUi: true })

    const status = hakemustenArviointiPage.hakemusRows.locator('.hakemus-status-cell')
    expect(await status.textContent()).toEqual('Ei tarvetta')

    const muutoshakemusStatus = hakemustenArviointiPage.hakemusRows.locator('[data-test-class=muutoshakemus-status-cell]')
    expect(await muutoshakemusStatus.textContent()).toEqual('-')

    const valiselvitysStatus = hakemustenArviointiPage.hakemusRows.locator('[data-test-class=valiselvitys-status-cell]')
    expect(await valiselvitysStatus.textContent()).toEqual('-')

    const loppuselvitysStatus = hakemustenArviointiPage.hakemusRows.locator('[data-test-class=loppuselvitys-status-cell]')
