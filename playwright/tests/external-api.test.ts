...
const test = muutoshakemusTest.extend({
  answers: ({answers}, use) => use({
    ...answers,
    hakemusFields: [
      { fieldId: 'project-nutshell', answer: nutshell }
    ],
  }),
  hakuProps: ({hakuProps}, use) => use({
    ...hakuProps,
    hakemusFields: [
      { fieldId: 'project-nutshell', type: 'textField' }
    ],
  })
})

test('external api', async ({ finalAvustushakuEndDate, page, avustushakuID, acceptedHakemus: { hakemusID } }) => {
