...
test.describe.parallel("Bank account validator", function () {
  test("can validate a valid IBAN number", function () {
    expect(BankAccountValidator.isValidIban("FI2112345600000785")).toEqual(
      true
    );
  });
  ...
  test("can recognize an invalid IBAN number", function () {
    expect(BankAccountValidator.isValidIban("FI2112345600000786")).toEqual(
      false
    );
  ...
  test("can recognize an invalid BIC", function () {
    ...
    expect(BankAccountValidator.isValidBic("Fi2112345600000785")).toEqual(
      false
    );
  });
});
