...
test.describe.parallel('VA trainee day utilities', () => {
  ...
  test.describe('summing subfield values', function () {
    ...
    test('sums subfields by subfield type', function () {
      expect(VaTraineeDayUtil.sumSubfieldValues(answers, 'person-count')).toEqual(73)
      expect(VaTraineeDayUtil.sumSubfieldValues(answers, 'total')).toEqual(643.5)
    })

    test('treats non-number as zero', function () {
      expect(VaTraineeDayUtil.sumSubfieldValues(answers, 'scope-type')).toEqual(0)
    })
  })

  test.describe('collecting calculator specifications', function () {
    const makeFormSpec = (calcSpecs: Record<string, string>[]) =>
      calcSpecs.map(({ growingFieldsetId, growingFieldsetChildId, calcFieldId, fiLabel }) => {
        return {
