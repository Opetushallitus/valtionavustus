...
test("multiple muutoshakemus", async ({
  page,
  ...
  await test.step(
    "when going again to muutoshakemus tab its still shown as rejected",
    async () => {
      await hakemustenArviointiPage.navigateToLatestMuutoshakemus(
        avustushakuID,
        hakemusID
      );
      await hakemustenArviointiPage.validateMuutoshakemusValues(
        muutoshakemus1,
        {
          status: "rejected",
        }
  ...
  await test.step(
    "muutoshakemus status is hyväksytty in hakemustenArviointiPage ",
    async () => {
      await hakemustenArviointiPage.navigate(avustushakuID);
      await expect(
        hakemustenArviointiPage.muutoshakemusStatusFieldContent()
  ...
  await test.step("in jatkoaika tab shos correct values", async () => {
    ...
    await hakemustenArviointiPage.page
      .locator("[data-test-id=muutoshakemus-jatkoaika]")
      .click();
    await hakemustenArviointiPage.validateMuutoshakemusValues(muutoshakemus2, {
      status: "accepted",
    });
  ...
  await test.step("in hakijaMuutoshakemusPage has correct values", async () => {
    await hakijaMuutoshakemusPage.navigate(hakemusID);
    expect(
      await hakijaMuutoshakemusPage.existingMuutoshakemusLocator.allInnerTexts()
    ).toHaveLength(4);
    ...
    expect(
      await hakijaMuutoshakemusPage.page
        .locator('[data-test-id="icon-rejected"]')
        .elementHandles()
    ).toHaveLength(2);
  });
  ...
  await test.step(
    "navigate with tab and check muutoshakemus #3 values",
    async () => {
      await hakemustenArviointiPage.page
  ...
  await test.step(
    "navigate with tab and check muutoshakemus #2 values",
    async () => {
      await hakemustenArviointiPage.page
        .locator('[data-test-id="muutoshakemus-tab-2"]')
        .click();
      await hakemustenArviointiPage.validateMuutoshakemusValues(
        muutoshakemus2,
        { status: "accepted" }
  ...
  await test.step(
    "navigate with tab and check muutoshakemus #1 values",
    async () => {
      await hakemustenArviointiPage.page
        .locator('[data-test-id="muutoshakemus-tab-3"]')
        .click();
      await hakemustenArviointiPage.validateMuutoshakemusValues(
        muutoshakemus1,
        { status: "rejected" }
