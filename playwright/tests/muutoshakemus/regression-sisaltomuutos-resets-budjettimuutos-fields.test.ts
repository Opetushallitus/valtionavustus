...
test('When making a päätös for muutoshakemus, changing the status of other parts should not reset budjetti', async ({hakemustenArviointiPage}) => {
  ...
  await test.step('fill in amended budget', async () => {
    await hakemustenArviointiPage.setMuutoshakemusBudgetDecision('accepted_with_changes', changedBudgetAmounts)
