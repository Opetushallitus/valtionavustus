...
const muutoshakuDisabledMenoluokiteltuLomakeJson = fs.readFileSync(
  path.join(
    __dirname,
    "../../fixtures/muutoshakemus-disabled-menoluokiteltu.hakulomake.json"
  ),
  "utf8"
);
  ...
  hakemusID: async (
    ...
    await test.step("fill info", async () => {
      ...
      await hakijaAvustusHakuPage.page.click(
        'label[for="radioButton-0.radio.0"]'
      ...
      await hakijaAvustusHakuPage.page.click(
        'label[for="combined-effort.radio.1"]'
      ); // yhteishanke = ei
      ...
      await hakijaAvustusHakuPage.page.click(
        'label[for="radioButton-1.radio.0"]'
      ); // Teema: Johtamisosaamisen ja yhteisöllisen kehittämisen vahvistaminen
      await hakijaAvustusHakuPage.page.click(
        'label[for="radioButton-2.radio.0"]'
      ); // Mistä muista kohderyhmistä koulutukseen voi osallistua: Varhaiskasvatus
      await hakijaAvustusHakuPage.page.fill(
        "#project-goals",
      ...
      await hakijaAvustusHakuPage.page.fill(
        "#textArea-5",
        "Päästä matkustamaan tulevaisuuteen"
      ...
      await hakijaAvustusHakuPage.page.fill(
        "#project-announce",
        "Innostamalla"
      ); // Miten osallistujat rekrytoidaan koulutukseen?
      await hakijaAvustusHakuPage.page.fill(
      ...
      await hakijaAvustusHakuPage.page.click(
        'label[for="vat-included.radio.0"]'
      ); // Onko kustannukset ilmoitettu arvonlisäverollisina
      await hakijaAvustusHakuPage.page
    ...
    await test.step("fill budget", async () => {
      await hakijaAvustusHakuPage.page
        ...
        .fill("Liksat");
      await hakijaAvustusHakuPage.page
        .locator("[id='personnel-costs-row.amount']")
        .fill("666");
      await hakijaAvustusHakuPage.page
        .locator("[id='material-costs-row.description']")
        .fill("Lakupiippuja");
      await hakijaAvustusHakuPage.page
        .locator("[id='material-costs-row.amount']")
        .fill("4");
      await hakijaAvustusHakuPage.page
        .locator("[id='rent-costs-row.description']")
        .fill("Bajamaja");
      await hakijaAvustusHakuPage.page
        .locator("[id='rent-costs-row.amount']")
        .fill("14");
      await hakijaAvustusHakuPage.page
      ...
      await hakijaAvustusHakuPage.page
        .locator("[id='steamship-costs-row.amount']")
        .fill("0");
      await hakijaAvustusHakuPage.page
        .locator("[id='other-costs-row.description']")
        .fill("Banaani");
      await hakijaAvustusHakuPage.page
        .locator("[id='other-costs-row.amount']")
        .fill("10");
    });
...
akuTest(
  ...
  async ({ page, avustushakuID, hakemusID }) => {
    ...
    await test.step(
      "total myönnetty amount is displayed correctly",
      async () => {
        await expect(seuranta.kustannusMyonnetty).toHaveText("100000");
      }
    );
    await test.step(
      "OPH:n hyväksymä amount is displayed correctly",
      async () => {
