...
const test = muutoshakemusTest.extend<{
  ...
  hakemustenArviointiPage: async (
    { acceptedHakemus: { hakemusID }, avustushakuID, page },
    use
  ) => {
...
test("muutoshakemus päätös preview", async ({ hakemustenArviointiPage }) => {
  ...
  await test.step("accepted", async () => {
    ...
    await expect(preview.jatkoaikaPaatos).toHaveText(
      "Hyväksytään haetut muutokset käyttöaikaan"
    );
  ...
  await test.step("rejected", async () => {
    ...
    await preview.open();
    await expect(preview.jatkoaikaPaatos).toHaveText(
      "Hylätään haetut muutokset käyttöaikaan"
    );
    await expect(preview.sisaltoPaatos).toHaveText(
  ...
  async ({ hakemustenArviointiPage }) => {
    ...
    const assertCorrectExistingBudget = async () => {
      ...
      expect(sortedFormTable(currentValues)).toEqual(
        sortedFormTable(expectedExistingBudget)
      );
    };
    ...
    await test.step("accepted with changes", async () => {
      ...
      expect(sortedFormTable(changedCells)).toEqual(
        sortedFormTable(expectedChangedBudget)
      );
    ...
    await test.step("accepted", async () => {
      ...
      await expect(preview.talousarvioPaatos).toHaveText(
        "Hyväksytään haetut muutokset budjettiin"
      );
      await preview.close();
    });
    ...
    await test.step("rejected", async () => {
      ...
      await preview.open();
      await expect(preview.talousarvioPaatos).toHaveText(
        "Hylätään haetut muutokset budjettiin"
      );
    });
  }
);
