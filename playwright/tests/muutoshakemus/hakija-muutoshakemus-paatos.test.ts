...
test("Hakija views muutoshakemus page", async ({
  ...
  await test.step("Current budget title is shown in finnish", async () => {
    expect(await hakijaMuutoshakemusPaatosPage.currentBudgetTitle()).toEqual(
      "Vanha budjetti"
    );
  });
  ...
  await test.step("Päätöksen tekijä is shown in finnish", async () => {
    expect(await hakijaMuutoshakemusPaatosPage.paatoksetTekija()).toEqual(
      "Hyväksyjä"
    );
  });

  await test.step("Lisätietoja title is shown in finnish", async () => {
    expect(await hakijaMuutoshakemusPaatosPage.lisatietojaTitle()).toEqual(
      "Lisätietoja"
    );
  });
  ...
  await test.step("the old budget is shown on the päätös", async () => {
    ...
    const currentValues =
      await hakijaMuutoshakemusPaatosPage.existingBudgetTableCells();
...
svTest(
  ...
  async ({ hakijaMuutoshakemusPaatosPage }) => {
    ...
    await svTest.step("current budget title is shown in swedish", async () => {
      const currentBudgetHeader =
        await hakijaMuutoshakemusPaatosPage.currentBudgetTitle();
    ...
    await svTest.step(
      ...
      async () => {
        const budgetChangeText =
          await hakijaMuutoshakemusPaatosPage.infoSection();
        expect(budgetChangeText).toEqual(
          "Ändringsansökan som gäller projektets budget"
        );
      }
    );
    ...
    await svTest.step("Budget rows are in Swedish", async () => {
      ...
      expect(swedishBudgetRowDescriptions.sort()).toEqual(
        swedishBudgetRowNames.sort()
      );
    });
  }
);
