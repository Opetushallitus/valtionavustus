...
test.describe.parallel(
  "Avustushaku that was marked as muutoshakukelvoton",
  () => {
    test("turns into muutoshakukelpoinen when copied", async ({
      avustushakuID,
      page,
    }) => {
      ...
      expect(
        await page.$('[data-test-id="muutoshakukelvoton-warning"]')
      ).toBeNull();
    });

    test("shows warning on Haun tiedot tab", async ({
      avustushakuID,
      page,
    }) => {
      const hakujenHallintaPage = new HakujenHallintaPage(page);
      ...
      expect(
        await hakujenHallintaPage.page.textContent(
          '[data-test-id="muutoshakukelvoton-warning"]'
        )
      ).toEqual(
    ...
    test("navigates to the old virkailija edit view", async ({
      ...
      expect(
        await modificationPage.evaluate(() => window.location.href)
      ).toMatch(
    ...
    test("sends a notification to hakija when yhteyshenkilö is changed", async ({
      ...
      await hakijaAvustusHakuPage.navigateToYhteyshenkilöChangePage(
        avustushakuID,
        userKey,
        token
      );
      ...
      await waitForElementWithText(
        page,
        "button",
        "Aloita yhteystietojen muokkaus"
      );
      ...
      await waitForElementWithText(
        page,
        "button",
        "Tallenna muutos ja lopeta muokkaus"
      );
      await page.fill("#applicant-name", "Etunimi Takanimi");
      await page.waitForFunction(
        () =>
          (document.querySelector("#applicant-edit-submit") as HTMLInputElement)
            .disabled === false
      );
      await clickElementWithText(
        page,
