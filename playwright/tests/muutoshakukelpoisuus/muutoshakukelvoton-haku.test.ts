...
import { getAcceptedPäätösEmails, getHakemusTokenAndRegisterNumber, getYhteystiedotMuutettuEmails, lastOrFail, waitUntilMinEmails } from '../../utils/emails'
import { clickElementWithText, waitForElementWithText, waitForNewTab } from '../../utils/util'

test.describe('Avustushaku that was marked as muutoshakukelvoton', () => {
  ...
  test('shows warning on Haun tiedot tab',  async ({ avustushakuID, page }) => {
    ...
    expect(await hakujenHallintaPage.page.textContent('[data-test-id="muutoshakukelvoton-warning"]')).toEqual(
  ...
  test('does not send link to muutoshaku page with päätös', async ({ avustushakuID, acceptedHakemus: { hakemusID } }) => {
    ...
    expect(email).not.toContain('Pääsette tekemään muutoshakemuksen sekä muuttamaan yhteyshenkilöä ja hänen yhteystietojaan koko hankekauden ajan tästä linkistä')
    expect(email).toContain('Yhteyshenkilö vaihdetaan oheisen linkin kautta, joka on käytettävissä läpi avustuksen käyttöajan:')
    expect(email).toContain(`${HAKIJA_URL}/avustushaku/${avustushakuID}/nayta?avustushaku=${avustushakuID}`)
  })

  test('navigates to the old virkailija edit view', async ({ avustushakuID, acceptedHakemus: { userKey }, page }) => {
  ...
  test('sends a notification to hakija when yhteyshenkilö is changed', async ({ avustushakuID, acceptedHakemus: { hakemusID, userKey }, page }) => {
