...
test.describe("Form editor muutoshakukelpoisuus message", () => {
  test("tells that avustushaku is muutoshakukelpoinen", async ({
    avustushakuID,
    page,
  }) => {
    const hakujenHallintaPage = new HakujenHallintaPage(page);
    await hakujenHallintaPage.navigateToFormEditor(avustushakuID);
    const okBannerText = await page.innerText(
      '[data-test-id="muutoshakukelpoisuus-ok"]'
    );
  ...
  test("shows field ids on the hakemus edit page", async ({
    avustushakuID,
    page,
  }) => {
    const hakujenHallintaPage = new HakujenHallintaPage(page);
  ...
  test("tells user about one missing field", async ({ page, hakuProps }) => {
    ...
    await hakujenHallintaPage.createHakuWithLomakeJson(
      puuttuvaYhteyshenkilonNimiJson,
      hakuProps
    );
  ...
  test("tells user about multiple missing fields", async ({
    page,
    hakuProps,
  }) => {
    const hakujenHallintaPage = new HakujenHallintaPage(page);
...
const expectMissingFieldId = (page: Page, fieldId: string) =>
  page.waitForSelector(
    `.muutoshakukelpoisuus-dropdown-item-id >> text="${fieldId}"`
  );
const expectMissingFieldLabel = (page: Page, labelText: string) =>
  page.waitForSelector(
    `.muutoshakukelpoisuus-dropdown-item-label >> text="${labelText}"`
  );
