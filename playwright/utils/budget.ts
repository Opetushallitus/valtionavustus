...
export const getBudgetSelectorsForType = (
  ...
  return {
    ...
    'service-purchase': page.locator(`[id='${prefix}service-purchase-costs-row.${field}']`),
...
export const fillBudget = async (page: Page, budget: Budget, type: 'hakija' | 'virkailija') => {
  const prefix = type === 'virkailija' ? 'budget-edit-' : ''
  ...
  if (type === 'hakija') {
    await page.fill(`[id='${prefix}self-financing-amount']`, budget.selfFinancing)
  }
}
