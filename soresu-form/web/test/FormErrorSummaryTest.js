...
describe("Form full of errors", function () {
  ...
  beforeEach(function () {
    validationErrors = _(
      JsUtil.flatFilter(
        formContent,
        (x) => !_.isUndefined(x.id) && x.fieldClass === "formField"
      )
    )
      .map((field) => {
  ...
  it("gets its summary calculated", function () {
    ...
    assert.equal(
      privateFinancingIncomeEntry.closestParent.id,
      "private-financing-income-row",
      JSON.stringify(privateFinancingIncomeEntry)
    );
    assert.equal(
      privateFinancingIncomeEntry.errors[0].error,
      "required",
      JSON.stringify(privateFinancingIncomeEntry)
    );
  });
