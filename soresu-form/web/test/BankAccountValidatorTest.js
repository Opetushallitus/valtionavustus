...
describe("Bank account validator", function () {
  it("can validate a valid IBAN number", function () {
    expect(BankAccountValidator.isValidIban("FI2112345600000785")).to.equal(
      true
    );
  });
  ...
  it("can validate an IBAN number with spaces", function () {
    expect(
      BankAccountValidator.isValidIban("GB82 WEST 1234 5698 7654 32")
    ).to.equal(true);
  });

  it("can recognize an invalid IBAN number", function () {
    expect(BankAccountValidator.isValidIban("FI2112345600000786")).to.equal(
      false
    );
    expect(BankAccountValidator.isValidIban("I am not an IBAN")).to.equal(
      false
    );
  });
  ...
  it("can recognize an invalid BIC", function () {
    ...
    expect(BankAccountValidator.isValidBic("Fi2112345600000785")).to.equal(
      false
    );
  });
});
