...
describe("FormBranchEditableFieldGrower", () => {
    ...
    const answersObject = makeAnswersObject([makeGrowingFieldsetAnswer("growingFieldsetChild", [
      {
        ...
        value: [
          ...
          {key: "email", value: "u2@example.com", fieldType: "vaEmailNotification"}
        ]
      },
      {
        key: 3,
        value: [
          ...
          {key: "email", value: "u3@example.com", fieldType: "vaEmailNotification"}
        ]
      }
    ])])
  ...
  it("sets modified field required if the prototype field was required", () => {
    ...
    expect(FormUtil.findField(state.form.content, "my-fieldset-1.money").required).be.true
    expect(FormUtil.findField(state.form.content, "my-fieldset-3.money").required).be.false

    expect(state.form.validationErrors).to.eql({"my-fieldset-1.money": [{error: "required"}]})
  })
})

const makeAnswersObject = answers => (
  {value: answers}
)
...
const makeGrowingFieldsetSpec = (childFieldType, childKeys, childFields) => (
  {
    ...
    "id": "my-fieldset",
    "fieldType": "growingFieldset",
    "children": childKeys.map(key => makeGrowingFieldsetChildSpec(childFieldType, key, childFields))
  }
)
...
const makeGrowingFieldsetChildFieldSpec = (childKey, {key: answerKey, fieldType, required = false}) => (
  {
    "label": {
      "fi": "",
      "sv": ""
...
const makeGrowingFieldsetAnswer = (childFieldType, childValues) => (
  {
    "key": "my-fieldset",
    "value": childValues.map(val => makeGrowingFieldsetChildAnswer(childFieldType, val)),
    "fieldType": "growingFieldset"
  }
)
...
const makeGrowingFieldsetChildFieldAnswer = (childKey, {key: answerKey, value, fieldType}) => (
  {
...
const getFieldsetIdTree = formSpec =>
  FormUtil.findField(formSpec, "my-fieldset").children.map(c => ([c.id, c.children.map(c => c.id)]))
