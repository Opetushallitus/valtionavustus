...
export const MuutoshakemusValues = (props: MuutoshakemusValuesProps) => {
  const { currentTalousarvio, muutoshakemus, hakijaUrl, projectEndDate } =
    props;
  ...
  return (
    <React.Fragment>
      {hasAnyPaatos && (
        <section
          className="muutoshakemus-section"
          data-test-id="muutoshakemus-paatos"
        >
          <div className="muutoshakemus__paatos">
            <h2>
              <span data-test-id="paatos-status-text">
                {t.muutoshakemus.paatos.processed}
              </span>
              {a["paatos-created-at"] &&
                ` ${moment(a["paatos-created-at"]).format(
                  datetimeFormat(lang)
                )}`}
            </h2>
            <h3
              className="muutoshakemus__header"
              data-test-id="päätös-email-status"
            >
              {a["paatos-sent-at"]
                ? `${t.email.paatos.status.sent} ${moment(
                    a["paatos-sent-at"]
                  ).format(datetimeFormat(lang))}`
                : t.email.paatos.status.pending}
            </h3>
            <h3 className="muutoshakemus__header">
              {t.muutoshakemus.paatos.paatosDokumentti}:
            </h3>
            <a
              href={paatosUrl}
              target="_blank"
      ...
      {muutoshakemus["paatos-reason"] && (
        <div className="muutoshakemus-paatos-reason">
          <h2 className="muutoshakemus__header">
    ...
    <MuutoshakemusSection>
      ...
      <div className="muutoshakemus__project-end-row">
        ...
        <div>
          ...
          <div data-test-id="muutoshakemus-jatkoaika">
