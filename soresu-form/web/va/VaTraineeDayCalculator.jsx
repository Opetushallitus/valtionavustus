...
export default class VaTraineeDayCalculator extends BasicFieldComponent {
  ...
  static validateTotal(field, value) {
    ...
    const total = parseDecimal(
      VaTraineeDayUtil.readSubfieldValue(value, field.id, "total")
    );
  ...
  render() {
    ...
    const scopeStr = VaTraineeDayUtil.readSubfieldValue(
      valueHolder.value,
      field.id,
      "scope"
    );
    ...
    const totalStr = VaTraineeDayUtil.readSubfieldValue(
      valueHolder.value,
      field.id,
      "total"
    );
      ...
      <div id={htmlId} className="va-trainee-day-calculator">
        <table>
          <thead>
            <tr>
              <th>
                {this.translator.translate("scope-type", this.props.lang)}
              </th>
              <th>{this.translator.translate("scope", this.props.lang)}</th>
              <th>
                {this.translator.translate("person-count", this.props.lang)}
              </th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>
                <RadioButton
                  ...
                  onChange={onChange(
                    VaTraineeDayCalculator.subfieldSpecFor(
                      field.id,
                      "scope-type"
                    )
                  )}
              ...
              <td>
                <BasicTextField
                  ...
                  disabled={props.disabled}
                  onChange={onChange(
                    VaTraineeDayCalculator.subfieldSpecFor(field.id, "scope")
                  )}
                  value={scopeStr}
                  translations={{}}
...
export class VaTraineeDayTotalCalculator extends React.Component {
  ...
  render() {
    ...
    const sumPersonCount = VaTraineeDayUtil.sumSubfieldValues(
      calcAnswers,
      "person-count"
    );
