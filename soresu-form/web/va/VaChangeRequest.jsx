...
export default class VaChangeRequest extends React.Component {
  render() {
    ...
    if (
      hakemus &&
      (hakemus.status === "pending_change_request" ||
        hakemus.status === "officer_edit")
    ) {
      ...
      const dateStr =
        DateUtil.asDateString(hakemus["version-date"]) +
        " " +
        timeLimiter +
        " " +
        DateUtil.asTimeString(hakemus["version-date"]);
      const firstName = NameFormatter.onlyFirstForename(
        hakemus["user-first-name"]
      );
      const lastName = hakemus["user-last-name"];
      return (
        <div className="change-request">
          <div className="change-request-title">
            {translator.translate(hakemus.status, this.props.lang)} {dateStr}
            <span hidden={!firstName} title={firstName + " " + lastName}>
              {" "}
              ({NameFormatter.shorten(firstName, lastName)})
            </span>
          </div>
