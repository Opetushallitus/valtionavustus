...
export default class VaTraineeDayUtil {
  ...
  static composeTotal(scopeValue, personCountValue, scopeType) {
    ...
    const total = VaTraineeDayUtil.calculateTotal(
      parseScope(),
      parsePerconCount(),
      scopeType
    );
  ...
  static sumSubfieldValues(calculatorAnswers, subfieldType) {
    return _.reduce(
      calculatorAnswers,
      (sum, ans) => {
        const val = parseDecimal(
          VaTraineeDayUtil.readSubfieldValue(ans.value, ans.key, subfieldType)
        );
  ...
  static readSubfieldValue(subfields, fieldId, subfieldType) {
    const subfield = VaTraineeDayUtil.findSubfieldById(
      subfields,
      fieldId,
      subfieldType
    );
  ...
  static collectCalculatorSpecifications(formSpecificationContent, answers) {
    ...
    return _.flatten(
      _.map(
        FormUtil.findFieldsByFieldType(
          formSpecificationContent,
          "vaTraineeDayCalculator"
        ),
        (calcSpec) => {
          const idPrefix = calcSpec.id
            .split(".")
            .slice(0, -1)
            .join(".")
            .replace(/-\d+$/, "-");
  ...
  static findGrowingFieldsetChildByCalculatorId(answers, calcId) {
    const growingFieldsetChildren = JsUtil.flatFilter(answers, (n) => {
      return n.fieldType === "growingFieldsetChild";
    });
    return _.find(growingFieldsetChildren, (gfc) =>
      _.find(gfc.value, (ans) => ans.key === calcId)
    );
  }
}
