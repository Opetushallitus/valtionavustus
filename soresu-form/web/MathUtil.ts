...
export function decimalShareRoundedUpOf(
  fraction: BigSource,
  total: BigSource
): number {
...
export function ratioShareRoundedUpOf(ratio: Fraction, total: BigSource) {
  return decimalShareRoundedUpOf(
    Big(ratio.nominator).div(ratio.denominator),
    total
  );
}

export function percentageShareRoundedUpOf(
  percentage: number,
  total: BigSource
) {
...
export function roundDecimal(
  ...
  return Number(
    Math[roundingMode](Number(number + "e" + digits)) + "e-" + digits
  );
}
