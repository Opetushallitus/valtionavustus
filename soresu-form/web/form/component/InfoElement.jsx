...
export class BasicInfoComponent extends React.Component {
  asDateTimeString(date) {
    const timeLimiter = new Translator(this.props.translations['misc']).translate(
      'time',
      this.props.lang,
      'KLO'
    )
...
export class LinkInfoElement extends BasicInfoComponent {
  render() {
    ...
    return (
      <a hidden={!translatedHref} target="_blank" rel="noopener noreferrer" href={translatedHref}>
        {text}
      </a>
    )
  }
...
export class AccordionInfoElement extends BasicInfoComponent {
  ...
  render() {
    ...
    return (
      <div>
        <span onClick={this.handleClick} className={'soresu-opener-handle ' + openStateClassName}>
