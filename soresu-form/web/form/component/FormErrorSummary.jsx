...
export default class FormErrorSummary extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    const isUptoDate =
      ...
      _.reduce(
        oldIdsOfInvalidFields,
        function (acc, id) {
          return (
            acc &&
            _.isEqual(oldValidationErrors[id], nextProps.validationErrors[id])
          );
        },
        true
      );
  ...
  render() {
    ...
    const fieldsWithErrorsAndClosestParents =
    ...
    const fieldErrorMessageElements = _.map(
      fieldsWithErrorsAndClosestParents,
      (x) => {
    ...
    const openStateClassName = FormErrorSummary.determineCssClass(
      this.state.open
    );
    ...
    return (
      <div id="form-error-summary">
        <a
          ...
          role="button"
          className={
            "error soresu-opener-handle validation-errors-summary " +
            openStateClassName
          }
        >
  ...
  static resolveFieldsErrorsAndClosestParents(validationErrors, formContent) {
    ...
    return _.sortBy(results, (x) =>
      FormUtil.findIndexOfField(formContent, x.field.id)
    );
  }
  ...
  renderFieldErrors(formContent, field, closestParent, errors, lang) {
    ...
    return (
      <div
        className="error"
        key={htmlId + "-validation-error"}
        data-field-id={htmlId}
      >
