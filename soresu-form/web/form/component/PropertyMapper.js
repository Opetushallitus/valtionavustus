...
class CommonPropertyMapper {
  static map(props) {
    ...
    return {
      ...
      translations: props.translations.merge({
        label: field.label,
...
export class TextFieldPropertyMapper {
  static map(props) {
    ...
    const onBlur = BasicFieldComponent.checkValueOnBlur(
      field,
      props.htmlId,
      props.value,
      props.onChange,
...
export class UpperCaseTextFieldPropertyMapper {
  static map(props) {
    ...
    const onChange = (e) => {
      props.onChange(field, e.target.value.toUpperCase());
    };
    return _.extend(commonProps, {
      onChange: onChange,
    });
...
export class TrimmingTextFieldPropertyMapper {
  static map(props) {
    ...
    const commonProps = TextFieldPropertyMapper.map(props);
    const onChange = (e) => {
      props.onChange(field, _.trim(e.target.value));
    };
    return _.extend(commonProps, {
      onChange: onChange,
    });
  ...
  static map(props) {
    ...
    return _.extend(commonProps, {
      onChange: (selectedOption) => {
        props.onChange(props.field, selectedOption.value);
      },
    });
  }
}
...
export class AttachmentFieldPropertyMapper {
  static map(props) {
    ...
    const onDrop = (files) => {
      controller.uploadAttachment(props.field, files);
    };
    const onRemove = () => {
      controller.deleteAttachment(props.field);
    };
...
export class AccordionElementPropertyMapper {
  static map(props) {
    const commonProps = InfoElementPropertyMapper.map(props);
    const field = props.field;
    const initiallyOpen = DefaultPropertyMapper.param(
      field,
      "initiallyOpen",
      true
    );
