...
export function ensureFirstChildIsRequired(state: any, growingParent: Field) {
  ...
  const writeAnswersIfFormField = (f: Field) => {
    if (f.fieldClass === 'formField') {
      ...
      const existingInputValue = InputValueStorage.readValue(null, answersObject, f.id)
  ...
  const firstChildOfGrowingSet = makeImmutableChildrenMutable(growingParent.children[0])
  const flattenedC = JsUtil.flatFilter(firstChildOfGrowingSet, (n: Field) => !!n.id)
  ...
  clearValidationErrorsFromTheOriginalPositionOfMovedField(state, validationErrorsToDelete)

  const fieldsToValidate = flattenedC.filter((f) => f.fieldClass === 'formField')
