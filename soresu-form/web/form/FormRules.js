...
export default class FormRules {
  static applyRulesToForm(formSpecification, formState, values) {
    for (let i = 0; i < formSpecification.rules.length; i++) {
      ...
      switch (rule.type) {
        case "includeIf":
          formState = FormRules.handleIncludeIf(
            rule,
            formSpecification,
            formState,
            values
  ...
  static handleIncludeIf(rule, formSpecification, formState, values) {
    ...
    const doInclude =
      InputValueStorage.readValue(formState.content, values, triggerId) ===
      triggerValue;
  ...
  static addNewField(
    ...
    const newFieldPrototype = FormUtil.findField(
    ...
    parentField.children.splice(
      newFieldIndex,
      0,
      newFieldPrototype.asMutable({ deep: true })
    );
