...
export function initDefaultValues(values: Answers | {} | undefined, initialValues: any, formSpecificationContent: any, lang: Language) {
...
function loadSavedObjectPromise<T>(formOperations: FormOperations<T>, urlContent: UrlContent): EventStream<SavedObject | null> {
...
function getInitialFormValuesPromise<T>(formOperations: FormOperations<T>, formP: EventStream<Immutable.ImmutableObject<Form>>, initialValues: InitialValues, savedObjectP: EventStream<SavedObject | null>, lang: Language) {
  ...
  return valuesP.combine(formP, (values, form) => initDefaultValues(values, initialValues, form.content, lang))
}
...
export function initializeStateLoop<T extends BaseStateLoopState<T>, K>(controller: FormController<T, K>, formOperations: FormOperations<T>, initialValues: InitialValues, urlContent: UrlContent) {
  ...
  const initialFormStateP = initialValuesP.combine(formP, (values, form) =>
    FormRules.applyRulesToForm(form, { content: form.content.asMutable({deep: true}), validationErrors: Immutable({}) }, values)
  )
