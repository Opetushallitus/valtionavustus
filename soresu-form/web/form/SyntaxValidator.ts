...
export default class SyntaxValidator {
  static validateSyntax(
    ...
    switch (field.fieldType) {
      ...
      case "finnishBusinessIdField":
        {
          const finnishBusinessIdError =
            SyntaxValidator.validateBusinessId(value);
      ...
      default:
        if (customFieldSyntaxValidator) {
          const customError = customFieldSyntaxValidator.validateSyntax(
            field,
            value
          );
          if (customError) {
  ...
  static validateIban(input: any) {
    return BankAccountValidator.isValidIban(input)
      ? undefined
      : { error: "iban" };
  }

  static validateBic(input: any) {
    return BankAccountValidator.isValidBic(input)
      ? undefined
      : { error: "bic" };
  }
}
