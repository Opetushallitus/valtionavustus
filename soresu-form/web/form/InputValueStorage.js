...
export default class InputValueStorage {
  static writeValue(formSpecificationContent, answersObject, fieldUpdate) {
    ...
    function writeChildValue(parentObject, childField, value) {
      ...
      const existingChildren = _.filter(
        parentObject.value,
        (childValueObject) => {
          return childValueObject.key === childKey;
        }
      );
    ...
    let growingParentExistingValues = JsUtil.flatFilter(answersObject, (n) => {
      return n.key === growingParent.id;
    });
    if (_.isEmpty(growingParentExistingValues)) {
      writeChildValue(answersObject, growingParent, []);
      growingParentExistingValues = JsUtil.flatFilter(answersObject, (n) => {
        return n.key === growingParent.id;
      });
    }
    const repeatingItemField = JsUtil.findJsonNodeContainingId(
      growingParent.children,
      fieldId
    );
    let repeatingItemExistingValues = JsUtil.flatFilter(
      growingParentExistingValues,
      (n) => {
        return n.key === repeatingItemField.id;
      }
    );
    if (_.isEmpty(repeatingItemExistingValues)) {
      writeChildValue(growingParentExistingValues[0], repeatingItemField, []);
      repeatingItemExistingValues = JsUtil.flatFilter(answersObject, (n) => {
        return n.key === repeatingItemField.id;
      });
    }
  ...
  static readValue(_ignoredFormSpecificationContent, answersObject, fieldId) {
    ...
    const comparisonFun =
      _.isObject(firstElem) && firstElem.key
  ...
  static deleteValue(growingParent, answersObject, idOfFieldToRemove) {
    const parentValues = JsUtil.flatFilter(answersObject, (n) => {
      return n.key === growingParent.id;
    });
    if (!_.isEmpty(parentValues)) {
      _.remove(parentValues[0].value, (row) => {
        return row.key === idOfFieldToRemove;
      });
    }
  }
}
