  ...
  render() {
    ...
    const renderField = function (field, renderingParameters) {
      ...
      if (field.fieldClass === "infoElement") {
        ...
        const isOpened = () =>
          "applicant_edit" === _.get(state.saveStatus.savedObject, "status");

        let fieldDisabled;
        if (modifyApplication) {
          fieldDisabled =
            (isOpened() && !applicantFields.includes(field.id)) || !isOpened();
        } else {
          fieldDisabled =
            (formOperations.isFieldEnabled &&
              !formOperations.isFieldEnabled(field.id)) ||
        ...
        const extendedProperties = _.extend(fieldProperties, {
          ...
          attachmentUploadsInProgress:
            state.saveStatus.attachmentUploadsInProgress,
        });
    ...
    function createWrapperElement(field, fieldProperties, renderingParameters) {
      ...
      for (let i = 0; i < field.children.length; i++) {
        children.push(
          renderField(field.children[i], resolveChildRenderingParameters(i))
        );
      }

      return (
        <WrapperComponent {...fieldProperties} answersObject={values}>
          {children}
        </WrapperComponent>
      );
    }
