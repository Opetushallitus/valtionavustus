...
export default class YhteenvetoController {
  initializeState(parsedRoute) {
    ...
    const initialStateTemplate = {
      hakuData: Bacon.fromPromise(
        HttpUtil.get("/api/avustushaku/" + avustushakuId)
      ),
      savedSearchQuery: Bacon.fromPromise(
        HttpUtil.get(
          "/api/avustushaku/" + avustushakuId + "/searches/" + savedSearchId
        )
      ),
      hakemusFilter: {
        organization: "",
    ...
    return Bacon.update(
      {},
      [dispatcher.stream(events.initialState)],
      this.onInitialState,
      [dispatcher.stream(events.setFilter)],
      this.onFilterSet,
      [dispatcher.stream(events.setSorter)],
      this.onSorterSet
    );
  }
  ...
  onInitialState(emptyState, realInitialState) {
    ...
    realInitialState.hakuData.hakemukset = _.map(
      savedSearchQuery["hakemus-ids"],
      (id) => {
        return _.find(realInitialState.hakuData.hakemukset, (h) => {
          return h.id === id;
        });
      }
    );
