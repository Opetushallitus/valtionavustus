...
export function createAverageSummaryText(
  ...
  function createSummaryText() {
    ...
    return (
      textFromOthersResults +
      (average ? " - oma arviosi: " + meanToDisplay(average) : "")
    );
  }
  ...
  function meanToDisplay(meanScore: number) {
    return (
      1 +
      Math.round(10 * meanScore) / 10.0 +
      " (" +
      scoreToFI(Math.round(meanScore)) +
      ")"
    );
  }
}
...
function belongsToUser(
  scoreAverageByUser: PersonScoreAverage,
  userInfo: UserInfo
): boolean {
  return (
    scoreAverageByUser &&
    scoreAverageByUser["person-oid"] === userInfo["person-oid"]
  );
}
