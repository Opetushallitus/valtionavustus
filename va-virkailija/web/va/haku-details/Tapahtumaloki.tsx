...
export const Tapahtumaloki = ({ lahetykset }: TapahtumalokiProps) => {
  const dateTime = (d: string) =>
    `${DateUtil.asDateString(d)} ${DateUtil.asTimeString(d)}`;
  ...
  const sentMails = (entries: Lahetys[]) =>
    entries.filter((e) => e.success).length;
  const failedMails = (entries: Lahetys[]) =>
    entries.filter((e) => !e.success).length;

  // List of lists, each of which contain all sent emails sent by one transaction (i.e. batch_id), e.g. "lähetä päätökset"
  const groupedLahetykset = Object.values(
    groupLahetyksetByBatchId(lahetykset)
  ).sort(latestFirst);

  return (
    <div className={"tapahtumaloki"}>
      <div className={"header"}>{`Lähetetyt ${typeTranslation(
        lahetykset[0].tyyppi
      )}`}</div>
      <div className={"entries"}>
        <div className={"header-row"}>
          ...
          <span className={"failedCount header"}>
            Epäonnistuneita lähetyksiä
          </span>
        </div>
