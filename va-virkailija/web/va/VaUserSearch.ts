...
export function useVaUserSearch(): [string, React.Dispatch<React.SetStateAction<string>>, VaUserSearch] {
  ...
  const fetchData = async (searchInput: string) => {
    try {
      ...
      const result = await HttpUtil.post("/api/va-user/search", { searchInput })
      setVaUserSearch({ ...vaUserSearch, loading: false, result })
    } catch (e: unknown) {
      setVaUserSearch({ ...vaUserSearch, loading: false, result: { error: true, results: [] }})
    }
  }
  ...
  useEffect(() => {
    if (searchInput.length < minimumSearchInputLength) {
      setVaUserSearch({ ...vaUserSearch, loading: false, result: { error: false, results: [] }})
    } else {
