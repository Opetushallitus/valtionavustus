...
import { Hakemus, Muutoshakemus } from 'va-common/web/va/types'
import { Talousarvio } from 'va-common/web/va/types/muutoshakemus'
...
const renderBudgetRow = (avustushakuMeno: AvustushakuMeno, hakemus?: Hakemus, talousarvio?: Talousarvio) => {
  const meno = talousarvio?.find(t => t.type === avustushakuMeno.type)
  const grantedMeno = hakemus?.arvio['overridden-answers']?.value.find(a => a.key === `${avustushakuMeno.type}.amount`)?.value
  const grantedDescription = hakemus?.arvio['overridden-answers']?.value.find(a => a.key === `${avustushakuMeno.type}.description`)?.value
  const valiselvitysMeno = hakemus?.selvitys?.valiselvitys.answers?.find(a => a.key === `${avustushakuMeno.type}.amount`)?.value
  const valiselvitysDescription = hakemus?.selvitys?.valiselvitys.answers?.find(a => a.key === `${avustushakuMeno.type}.description`)?.value
  ...
  return (
    <tr key={avustushakuMeno.type} id="budget-edit-personnel-costs-row" className="budget-item" data-test-id={avustushakuMeno.type}>
      ...
      <td className="amount-column"><span className="money">{meno?.amount}</span></td>
      ...
      <form className="soresu-form">
        <fieldset id="budget-edit-budget" className="va-budget">
          <table id="budget-edit-project-budget" className="summing-table">
            <caption><span>Talousarvio</span></caption>
            <colgroup>
            ...
            <thead>
              <tr>
                <th className="label-column"><span>Menot</span></th>
                <th className="granted-amount-column">Myön­netty</th>
                {isValiselvitys && <th className="valiselvitys-amount-column money">Väli­selvitys</th>}
                {isLoppuselvitys && <th className="loppuselvitys-amount-column money">Loppu­selvitys</th>}
