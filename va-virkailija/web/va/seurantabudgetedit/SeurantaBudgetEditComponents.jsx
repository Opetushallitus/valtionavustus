...
export class EditSummingBudgetElement extends React.Component {
  render() {
    const { field, children, htmlId, lang, controller, customProps } =
      this.props;
    const sum = field.sum;
    ...
    const valiselvitysSum = EditSummingBudgetElement.getSelvitysSum(
      field,
      _.get(customProps.originalHakemus, "selvitys.valiselvitys", [])
    );
    const loppuselvitysSum = EditSummingBudgetElement.getSelvitysSum(
      field,
      _.get(customProps.originalHakemus, "selvitys.loppuselvitys", [])
    );
    ...
    return (
      <table id={htmlId} className="summing-table">
        ...
        <colgroup>
          ...
          {visibleColumns.grantedSum && (
            <col className="granted-amount-column" />
          )}
          {visibleColumns.valiselvitysSum && (
            <col className="valiselvitys-amount-column" />
          )}
          {visibleColumns.loppuselvitysSum && (
        ...
        <tfoot>
          <tr>
            <td className="label-column">
              <LocalizedString
                translations={field.params}
                translationKey="sumRowLabel"
                lang={lang}
              />
            </td>
            {visibleColumns.grantedSum && (
              <td className="granted-amount-column">
                <span className="money">{grantedSum.sum}</span>
              </td>
            )}
            {visibleColumns.valiselvitysSum && (
              <td className="valiselvitys-amount-column">
                <span className="money">{valiselvitysSum.sum}</span>
              </td>
            )}
            {visibleColumns.loppuselvitysSum && (
              <td className="loppuselvitys-amount-column">
                <span className="money">{loppuselvitysSum.sum}</span>
              </td>
            )}
  ...
  static columnTitles(field, lang, visibleColumns) {
    return (
      field.params.showColumnTitles && (
        <thead>
          <tr>
            <th className="label-column">
              <LocalizedString
                translations={field.params.columnTitles}
                translationKey="label"
                lang={lang}
              />
            </th>
            {visibleColumns.grantedSum && (
              <th className="granted-amount-column">Myön&shy;netty</th>
            )}
            {visibleColumns.valiselvitysSum && (
...
export class EditBudgetItemElement extends React.Component {
  render() {
    const { field, children, htmlId, disabled, lang, controller, customProps } =
      this.props;
    ...
    const showDetailedCosts =
      controller.budgetBusinessRules.showDetailedCostsForBudgetField(field);

    const grantedAnswers =
      customProps.originalHakemus.arvio["overridden-answers"];
    const valiselvitysAnswers = _.get(
      customProps.originalHakemus,
      "selvitys.valiselvitys",
      []
    );
    const loppuselvitysAnswers = _.get(
      customProps.originalHakemus,
      "selvitys.loppuselvitys",
      []
    );
    ...
    return (
      <tr id={htmlId} className="budget-item">
        ...
        {(!showDetailedCosts || !_.isEmpty(grantedAnswers)) && (
          <GrantedAmountCell
            answers={grantedAnswers}
            valueId={valueId}
            descriptionId={descriptionId}
            showContents={showDetailedCosts}
          />
        )}
        {!_.isEmpty(valiselvitysAnswers) && (
          <SelvitysAmountCell
            answers={valiselvitysAnswers}
            valueId={valueId}
            descriptionId={descriptionId}
            cellClassName="valiselvitys-amount-column"
          />
        )}
        {!_.isEmpty(loppuselvitysAnswers) && (
          <SelvitysAmountCell
            answers={loppuselvitysAnswers}
            valueId={valueId}
            descriptionId={descriptionId}
            cellClassName="loppuselvitys-amount-column"
          />
        )}
...
class GrantedAmountCell extends EditBudgetItemCell {
  render() {
    ...
    const cellClassNames = ClassNames("granted-amount-column", {
      "has-title": amount.hasDescription,
    });
...
class SelvitysAmountCell extends EditBudgetItemCell {
  render() {
    ...
    const cellClassNames = ClassNames(cellClassName, {
      "has-title": amount.hasDescription,
    });
    const moneyClassNames = ClassNames({
      "error error-message": !amount.hasValidValue,
