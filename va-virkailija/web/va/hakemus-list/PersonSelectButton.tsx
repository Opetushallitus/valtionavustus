...
export const PersonSelectButton = ({
  controller,
  hakemus,
  state,
  toggleSplitView,
}: PersonSelectButtonProps) => {
  const onClick = () => {
  ...
  const personCount =
    hakemus.arvio.roles.evaluators.filter((id) => roles.includes(id)).length +
    (presenterRoleId && roles.includes(presenterRoleId) ? 1 : 0);
