...
const RoleButton = ({role,roleField,controller,hakemus}) => {
  const onClick = (roleId) => controller.toggleHakemusRole(roleId,hakemus,roleField)
...
const PersonSelectPanel = ({hakemus,state,controller}) =>{
  ...
  return (
    <div hidden={!show} className="panel person-panel person-panel--top">
...
const PersonSelectButton = ({controller,hakemus,state,show}) => {
  ...
  return(
    <div hidden={!show}>
      <button onClick={onClick.bind(this,hakemus)} className="btn btn-sm btn-simple btn-role"><span className="btn-role__count">{countIndicator}</span></button>
