...
const hakemusSorter = ({
                         sortKey,
                         sortOrder,
                       }: SortState) => (a: Hakemus, b: Hakemus): number => {
    ...
    <table className={styles.hakemusTable}>
      ...
      <tr>
        <th>
          <div className={styles.tableHeader}>
            <input placeholder="Hakijaorganisaatio"
                   onChange={onOrganizationInput} value={organization}/>
            <SortButton sortKey="organization" sortingState={sortingState}
                        setSorting={setSorting} text="hakijaorganisaatio"/>
          </div>
        </th>
        <th>
          <div className={styles.tableHeader}>
            <input placeholder="Asianumero tai hanke" onChange={onProjectInput}
                   value={projectNameOrCode}/>
            <SortButton sortKey="registerNumber" sortingState={sortingState}
                        setSorting={setSorting} text="asianumero"/>
          </div>
        </th>
        <th>
          <div className={styles.tableHeader}>
            <TableLabel text="Arvio" disabled/>
            <SortButton sortKey="score" sortingState={sortingState}
                        setSorting={setSorting} text="arvio"/>
          </div>
        </th>
        <th>
          <div className={styles.tableHeader}>
            <StatusTableLabel
              ...
              onCheck={status => dispatch({
                type: 'set-status-filter',
                filter: 'hakemus',
                value: status
              })}
              onUncheck={status => dispatch({
              ...
              showDeleteButton={
                statusFilter.hakemus.length !== HakemusArviointiStatuses.statuses.length
                  ...
                  : undefined}
            />
            <SortButton sortKey="hakemus" setSorting={setSorting}
                        sortingState={sortingState} text="hakemuksen tila"/>
          </div>
        </th>
        <th>
          <div className={styles.tableHeader}>
            <TableLabel text="Haettu" disabled/>
            <SortButton sortKey="applied" sortingState={sortingState}
                        setSorting={setSorting} text="haettu summa"/>
          </div>
        </th>
        <th>
          <div className={styles.tableHeader}>
            <TableLabel text="Myönnetty" disabled/>
            <SortButton sortKey="granted" sortingState={sortingState}
                        setSorting={setSorting} text="myönnetty summa"/>
          </div>
        </th>
        <th>
          <PersonTableLabel
      ...
      {filteredList.map(hakemus => {
        ...
        return (
          <tr key={`hakemus-${hakemus.id}`}
            ...
            <td
              className="organization-cell">{hakemus["organization-name"]}</td>
            <td className="project-name-cell">
              <div className={styles.projectTd}>
                {getProject(hakemus)}
                {modified &&
                <Pill color="blue" testId={`${hakemus.id}-modified-pill`}
                      text="Muokattu" compact/>}
            ...
            <td className="hakemus-status-cell">{draft ? <EmptyGreyPill/> :
              <ArvioStatus status={hakemus.arvio.status}
                           refused={hakemus.refused}/>}</td>
            <td
              className={`${styles.alignRight} applied-sum-cell`}>{euroFormatter.format(hakemus["budget-oph-share"])}</td>
            ...
            <td className={styles.alignCenter}>
              ...
              <PersonSelectPanel hakemus={hakemus} state={state}
                                 controller={controller}/>}
            </td>
          </tr>
        )
      ...
      <tr>
        <td colSpan={4}>
          ...
          <a className={styles.yhteenveto} href="/yhteenveto/" target="_blank"
             onClick={() => onYhteenvetoClick(filteredList)}>Päätöslista</a>
        </td>
    ...
    <table className={styles.hakemusTable}>
      ...
      <tr>
        <th>
          <div className={styles.tableHeader}>
            <input placeholder="Hakijaorganisaatio"
                   onChange={onOrganizationInput} value={organization}/>
            <SortButton sortKey="organization" setSorting={setSorting}
                        sortingState={sortingState} text="hakijaorganisaatio"/>
          </div>
        </th>
        <th>
          <div className={styles.tableHeader}>
            <input placeholder="Asianumero tai hanke" onChange={onProjectInput}
                   value={projectNameOrCode}/>
            <SortButton sortKey="registerNumber" setSorting={setSorting}
                        sortingState={sortingState} text="asianumero"/>
          </div>
        </th>
        <th>
          <div className={styles.tableHeader}>
            ...
            <StatusTableLabel
              ...
              showDeleteButton={
                statusFilter.muutoshakemus.length !== Muutoshakemus.statuses.length
                  ...
                  : undefined}
            />
            <SortButton sortKey="muutoshakemus" setSorting={setSorting}
                        sortingState={sortingState}
            ...
            <StatusTableLabel
              ...
              showDeleteButton={
                statusFilter.valiselvitys.length !== HakemusSelvitys.statuses.length
                  ...
                  : undefined}
            />
            <SortButton sortKey="valiselvitys" setSorting={setSorting}
                        sortingState={sortingState}
            ...
            <StatusTableLabel
              ...
              showDeleteButton={
                statusFilter.loppuselvitys.length !== Loppuselvitys.statuses.length
                  ...
                  : undefined}
            />
            <SortButton sortKey="loppuselvitys" setSorting={setSorting}
                        sortingState={sortingState}
        ...
        <th>
          <div className={styles.tableHeader}>
            <TableLabel text="Myönnetty" disabled/>
            <SortButton sortKey="granted" setSorting={setSorting}
                        sortingState={sortingState} text="myönnetty summa"/>
          </div>
        </th>
        <th>
          <PersonTableLabel
      ...
      {filteredList.map(hakemus => (
        <tr key={`hakemus-${hakemus.id}`}
          ...
          <td className="hakemus-status-cell"><ArvioStatus
            status={hakemus.arvio.status} refused={hakemus.refused}/></td>
          <td data-test-class="muutoshakemus-status-cell"><MuutoshakemusPill
            status={hakemus["status-muutoshakemus"]} refused={hakemus.refused}/>
          </td>
          <td data-test-class="valiselvitys-status-cell"><ValiselvitysPill
            status={hakemus["status-valiselvitys"]} refused={hakemus.refused}/>
          </td>
          <td data-test-class="loppuselvitys-status-cell"><LoppuselvitysPill
            status={hakemus["status-loppuselvitys"]} refused={hakemus.refused}/>
          </td>
          <td className={`${styles.alignRight} granted-sum-cell`}>{
            hakemus.arvio["budget-granted"]
              ? euroFormatter.format(hakemus.arvio["budget-granted"])
...
const SortButton = ({
  ...
  return (
    <button aria-label={ariaLabel} className={buttonStyles.transparentButton}
            onClick={() => setSorting(sortKey)}
            data-test-id={`sort-button-${sortKey}`}>
      <svg width="12px" height="12px" viewBox="0 0 8 8" fill="none"
...
const TableLabel: React.FC<TableLabelProps> = ({
  ...
  return (
    <div className={styles.tableLabel}>
      <button disabled={!!disabled} onClick={() => toggleMenu(state => !state)}
              className={classNames(styles.tableLabelButton, {[buttonStyles.selected]: toggled})}>{text}</button>
      {showDeleteButton && (
    ...
    <div className={styles.tableLabel}>
      ...
      {toggled && (
        <div className={popupStyle} ref={ref}>
          <ControlledSelectPanel roles={roles} onClickClose={onOutsideClick}
                                 onClickRole={onClickRole} roleField={roleField}
                                 activeId={activeId}/>
        </div>
      )}
    </div>
...
function StatusTableLabel<Status extends Statuses>({
  ...
  return <TableLabel text={text} showDeleteButton={showDeleteButton}>
    {statuses.map(status => {
      ...
      return (
        <div key={`muutoshakemus-status-${status}`}
          ...
          <label
            htmlFor={status}>{labelText(status)} ({amountOfStatus(status)})</label>
        </div>
      )
    })}
  </TableLabel>
...
function ArvioStatus({
                       status,
                       refused
                     }: { status: HakemusArviointiStatus, refused: boolean | undefined }) {
  if (refused) {
...
function MuutoshakemusPill({
                             status,
                             refused
                           }: { status: MuutoshakemusStatus | undefined, refused: boolean | undefined }) {
  if (!status || refused) {
...
function ValiselvitysPill({
                            status,
                            refused
                          }: { status: SelvitysStatus | undefined, refused: boolean | undefined }) {
