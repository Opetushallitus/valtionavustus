...
export default class HakemusListing extends Component {
  static _fieldGetter(fieldName, userInfo, allowHakemusScoring) {
    switch (fieldName) {
      ...
      case "status_muutoshakemus":
        return (hakemus) =>
          hakemus.muutoshakemukset && hakemus.muutoshakemukset[0]
      ...
      case "score":
        return (hakemus) => {
          const score = effectiveAverage(
            hakemus.arvio.scoring,
            userInfo,
            allowHakemusScoring
          );
  ...
  static _filterAnswers(fieldGetter, filters) {
    ...
    return function (hakemus) {
      ...
      const filtersWithoutExcluded = filters.filter(
        (answer) => answer.id !== "rahoitusalue" && answer.id !== "tags"
      );
      const answerMatchPredicate = (filter) =>
        _.find(
          answers,
          (a) => a.key === filter.id && a.value === filter.answer
        );
      const groupedByQuestion = _.groupBy(filtersWithoutExcluded, "id");
      const filterValuesByQuestion = _.values(groupedByQuestion);
      const questionMatches = filterValuesByQuestion.map((v) =>
        _.some(v, answerMatchPredicate)
      );
  ...
  static _filterWithStrPredicate(fieldGetter, filter) {
    ...
    return function (hakemus) {
      ...
      return (
        fieldGetter(hakemus).toUpperCase().indexOf(filter.toUpperCase()) >= 0
      );
    };
  }
  ...
  static _filterRahoitusaluePredicate(fieldGetter, filters) {
    ...
    return function (hakemus) {
      ...
      if (_.isUndefined(fieldValue) || _.isNull(fieldValue)) {
        return _.some(
          rahoitusAlueetFilter,
          (i) => i.answer === "Ei rahoitusaluetta"
        );
      }
      return _.includes(
        rahoitusAlueetFilter.map((i) => i.answer),
        fieldValue
      );
    };
  }
  ...
  static _filter(list, filter) {
    return _.filter(
      ...
      HakemusListing._filterWithStrPredicate(
        HakemusListing._fieldGetter("name"),
        filter.name
      )
    )
      .filter(
        HakemusListing._filterWithStrPredicate(
          HakemusListing._fieldGetter("organization"),
          filter.organization
        )
      )
      .filter(
        HakemusListing._filterWithArrayPredicate(
          HakemusListing._fieldGetter("status"),
          filter.status
        )
      )
      .filter(
        HakemusListing._filterWithArrayPredicate(
          HakemusListing._fieldGetter("status_loppuselvitys"),
          filter.status_loppuselvitys
        )
      )
      .filter(
        HakemusListing._filterWithArrayPredicate(
          HakemusListing._fieldGetter("status_valiselvitys"),
          filter.status_valiselvitys
        )
      )
      .filter(
        HakemusListing._filterWithArrayPredicate(
          HakemusListing._fieldGetter("status_muutoshakemus"),
          filter.status_muutoshakemus
        )
      )
      .filter(
        HakemusListing._filterAnswers(
          HakemusListing._fieldGetter("answers"),
          filter.answers
        )
      )
      .filter(
        HakemusListing._filterWithArrayFilterPredicate(
          HakemusListing._fieldGetter("evaluators"),
          filter.evaluator
        )
      )
      .filter(
        HakemusListing._filterWithNumberPredicate(
          HakemusListing._fieldGetter("presenter"),
          filter.presenter
        )
      )
      .filter(
        HakemusListing._filterRahoitusaluePredicate(
          HakemusListing._fieldGetter("rahoitusalue"),
          filter.answers
        )
      )
      .filter(
        HakemusListing._filterTagsPredicate(
          HakemusListing._fieldGetter("tags"),
          filter.answers
        )
      );
  }
  ...
  static _sortByArray(
    ...
    return function (hakemus) {
      const sortValue = array.indexOf(
        fieldGetter(hakemus, userInfo, allowHakemusScoring)
      );
    ...
    return function (list, sorter) {
      switch (sorter.field) {
        case "status":
          return _.sortBy(
            list,
            HakemusListing._sortByArray(
              (hakemus) => hakemus.arvio.status,
  ...
  static _sort(list, sorterList, userInfo, allowHakemusScoring) {
    return _.reduce(
      sorterList,
      HakemusListing._sortBy(userInfo, allowHakemusScoring),
      list
    );
  }
  ...
  render() {
    ...
    const filteredHakemusList = HakemusListing._sort(
      HakemusListing._filter(hakemusList, filter),
      sorter,
      userInfo,
      allowHakemusScoring
    );
    ...
    const notPayTitle =
      "Yhteensä: " + notPayCount + " kpl hakemuksia merkattu ei maksettavaksi.";
    const notPayValue = "(" + notPayCount + ")";

    const anyPostModified =
      hakemusList.find(
        (h) => h["submitted-version"] && h["submitted-version"] !== h.version
      ) !== null;

    const ophShareSum = HakemusListing.formatNumber(
      _.sum(filteredHakemusList.map((x) => x["budget-oph-share"]))
    );
    ...
    const budgetGrantedSum = HakemusListing.formatNumber(
      _.sum(filteredHakemusList.map((x) => x.arvio["budget-granted"]))
    );
    ...
    return (
      <div
        id="hakemus-listing"
        className="listing-table hakemus-list-container"
      >
            ...
            <tr>
              ...
              {!isResolved && (
                <ChangeRequestHeader
                  ...
                  sorter={sorter}
                  controller={controller}
                  hakemusList={filteredHakemusList}
                />
              )}
              {!isResolved && isAcademysize && (
                <th className="academysize-column">
                  Koko
                  <HakemusSorter
                    field="academysize"
                    sorter={sorter}
              ...
              {includesShouldNotPay && (
                <th
                  className="should-pay-notification-column"
                  title={notPayTitle}
                >
                  {notPayValue}
                </th>
              ...
              {!isResolved && (
                <th className="applied-sum-column">
                  Haettu{" "}
                  <HakemusSorter
                    field="applied-sum"
                    sorter={sorter}
              ...
              {isResolved && (
                <th className="selvitys-column">
                  ...
                  />
                  <HakemusSorter
                    field="status_loppuselvitys"
                    sorter={sorter}
                    controller={controller}
                  />
                </th>
              )}
              <th className="granted-sum-column">
                Myönnetty{" "}
                <HakemusSorter
                  field="granted-sum"
                  sorter={sorter}
              ...
              <th className="person-filter-column">
                <PersonFilterButton controller={controller} state={state} />
              </th>
            </tr>
          </thead>
          <tbody
            data-test-id="hakemus-list"
            className={hasSelected ? "has-selected" : ""}
          >
            {hakemusElements}
          </tbody>
        </table>
        <div className="listing-footer">
          <div className="total-applications-column">
            <ApplicationSummaryLink
              filteredHakemusList={filteredHakemusList}
              hakemusList={hakemusList}
              controller={controller}
            />
          </div>
          <div className="right-side">
            <div className="applied-sum-column">
              {!isResolved && <span className="money">{ophShareSum}</span>}
            </div>
            <div className="granted-sum-column">
              <span className="money">{budgetGrantedSum}</span>
            </div>
...
class ApplicationSummaryLink extends Component {
  render() {
    ...
    const linkText =
      filteredHakemusList.length + "/" + hakemusList.length + " hakemusta";
    return disabled ? (
      <span>{linkText}</span>
    ) : (
      <a
...
class HakemusSorter extends Component {
  ...
  isSortedByField() {
    return !_.isEmpty(
      _.find(this.props.sorter, (sorter) => sorter.field === this.props.field)
    );
  }
...
class ChangeRequestHeader extends HakemusSorter {
  ...
  render() {
    ...
    const kplChangeRequest = _.filter(
      hakemusList,
      HakemusListing._filterWithArrayPredicate(
        (hakemus) => hakemus.status,
        ["pending_change_request"]
      )
    ).length;
    const kplOfficerEdit = _.filter(
      hakemusList,
      HakemusListing._filterWithArrayPredicate(
        (hakemus) => hakemus.status,
        ["officer_edit"]
    ...
    const title =
      total > 0
        ? `${total} odottaa täydennystä (hakijalta:${kplChangeRequest}, virkailijalta:${kplOfficerEdit})`
        : "Ei avoimia täydennyspyyntöjä";
    return (
      <th
        className="change-request-column"
  ...
  render() {
    ...
    for (let i = 0; i < statusValues.length; i++) {
      ...
      const kpl = _.filter(
        hakemusList,
        HakemusListing._filterWithArrayPredicate(
          HakemusListing._fieldGetter(filterField),
          [status]
        )
      ).length;
      statuses.push(
    ...
    return (
      <div className="status-filter">
        ...
        <button
          ...
          alt="Poista tilojen rajaukset"
          title="Poista tilojen rajaukset"
          tabIndex="-1"
        />
        <div
          className="status-filter-popup popup-box-shadow"
...
class HakemusRow extends Component {
  shouldComponentUpdate() {
    return (
      this.props.hakemus === this.props.selectedHakemus ||
      this.props.hakemus === this.props.previouslySelectedHakemus
    );
  }
  render() {
    const {
    ...
    const thisIsSelected =
      hakemus === selectedHakemus || hakemus === previouslySelectedHakemus;
    ...
    const statusLoppuselvitys = Loppuselvitys.statusToFI(
      hakemus["status-loppuselvitys"]
    );
    const statusValiselvitys = HakemusSelvitys.statusToFI(
      hakemus["status-valiselvitys"]
    );
    const muutoshakemusStatus = HakemusListing._fieldGetter(
      "status_muutoshakemus"
    )(hakemus);
    const decoratedMuutoshakemusStatus = `${
      muutoshakemusStatus === "new" ? "&#9734; " : ""
    }${Muutoshakemus.statusToFI(muutoshakemusStatus)}`;
    const changeRequest =
      HakemusListing._fieldGetter("change-request")(hakemus);
    const statusComment = hakemus["status-comment"]
      ? ":\n" + hakemus["status-comment"]
      : "";
    const changeRequestTitle = changeRequest
      ? "Odottaa täydennystä" + statusComment
      : "";
    ...
    const showNotPayIcon =
      "should-pay" in hakemus.arvio &&
      hakemus.arvio["should-pay"] !== null &&
      !hakemus.arvio["should-pay"];

    return (
      <tr
        id={htmlId}
        className={rowClass}
        onClick={() => controller.selectHakemus(hakemus.id)}
      >
        <td
          className="organization-column"
          title={hakemus["organization-name"]}
        >
        ...
        {includesShouldNotPay && (
          <td className="should-pay-notification-column">
            ...
            />
            {hakemus.refused && (
              <span title={hakemus["refused-comment"]}>H</span>
            )}
          </td>
        )}
        {anyPostModified && (
        ...
        {!isResolved && isAcademysize && (
          <td className="academysize-column">{hakemus.arvio.academysize}</td>
        )}
        {!isResolved && (
          <td className="applied-sum-column">
            <span className="money">
              {HakemusListing.formatNumber(hakemus["budget-oph-share"])}
            </span>
          </td>
        )}
        {isResolved && (
          <td
            data-test-id="väliselvitys-column"
            className="selvitys-column selvitys-value"
          >
            {statusValiselvitys}
          </td>
        )}
        ...
        <td className="granted-sum-column">
          <span className="money">
            {HakemusListing.formatNumber(hakemus.arvio["budget-granted"])}
          </span>
        </td>
...
class Scoring extends Component {
  render() {
    ...
    const starElements = _.map(_.range(4), (indexOfStar) => {
      ...
      const needsScaling =
        normalizedMeanScore > indexOfStar &&
        normalizedMeanScore < indexOfStar + 1;
      if (needsScaling) {
