...
export default class HakemusScoring extends Component {
  render() {
    ...
    const allowSeeingOthersScores =
      ...
      !allowHakemusScoring;
    const showOthersScores =
      this.props.showOthersScores && allowSeeingOthersScores;

    const avustushaku = this.props.avustushaku;
    const helpTexts = this.props.helpTexts;
    const valintaperusteet = _.get(
      avustushaku,
      "content.selection-criteria.items"
    );
    const myOwnValintaPerusteRows = HakemusScoring.createValintaPerusteRows(
      allScoresOfHakemus,
      valintaperusteet,
    ...
    const othersScoreDisplays = showOthersScores
      ? HakemusScoring.createOthersScoreDisplays(
          allScoresOfHakemus,
          scoringOfHakemus,
          valintaperusteet,
          myUserInfo
  ...
  static createValintaPerusteRows(
    allScoresOfHakemus,
    valintaperusteet,
    personOid,
    allowHakemusScoring,
    ...
    let perusteIndex = 0;
    return _.map(valintaperusteet, (peruste) => {
      return (
        <ValintaPerusteRow
    ...
    function findScores(perusteIndex) {
      return _.filter(allScoresOfHakemus, (s) => {
        return s["selection-criteria-index"] === perusteIndex;
      });
    }
  }

  static createOthersScoreDisplays(
    allScoresOfHakemus,
    scoringOfHakemus,
    valintaperusteet,
    myUserInfo
  ) {
    const othersPersonOids = _.map(
      othersScorings(scoringOfHakemus, myUserInfo),
      (s) => {
        return s["person-oid"];
      }
    );
    return _.map(othersPersonOids, (oid) => {
      ...
      return (
        <table key={"peruste-list-of" + oid} className="valintaperuste-list">
          ...
          <tbody>
            {HakemusScoring.createValintaPerusteRows(
              allScoresOfHakemus,
              valintaperusteet,
              oid,
              false,
...
class ValintaPerusteRow extends Component {
  render() {
    ...
    const controller = this.props.controller;
    const scoreOfUser = _.find(allScoresOfThisPeruste, (s) => {
      return s["person-oid"] === personOid;
    });
  ...
  render() {
    ...
    return (
      <div className={classNames}>
        <a title={titleText} onClick={onClick}>
          {labelText}
        </a>
      </div>
    );
