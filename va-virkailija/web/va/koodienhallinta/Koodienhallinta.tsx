...
const OphTab: React.FC<OphTabProps> = ({
  valueType,
  selectedValueType,
  onClick,
}) => {
...
const OphTabs: React.FC<{
  selectedTab: ValueType;
  onTabSelect: (valueType: ValueType) => void;
}> = ({ selectedTab, onTabSelect }) => {
  return (
    <div className="oph-tabs oph-typography">
      <OphTab
        valueType="operational-unit"
        selectedValueType={selectedTab}
        onClick={onTabSelect}
      />
      <OphTab
        valueType="project"
...
const CodeInputForm = withFormik<
  { selectedTab: ValueType; onActionSuccess: () => void },
  CodeInputValues
>({
  ...
  handleSubmit: async (values, actions) => {
    try {
      await HttpUtil.post("/api/v2/va-code-values/", {
        ...values,
    ...
    <div className="koodienhallinta-container">
      <div className="koodienhallinta-body">
        <OphTabs
          ...
          onTabSelect={(tab) => dispatch({ type: "select-tab", value: tab })}
        />
        <CodeInputForm
          selectedTab={selectedTab}
          onActionSuccess={() => dispatch({ type: "trigger-code-refetch" })}
        />
        <hr />
        <CodeFilter
          selectedTab={selectedTab}
          ...
          onSelect={(year) =>
            dispatch({ type: "set-filter-year", value: year })
          }
          onFilterWord={(word) =>
            dispatch({ type: "set-filter-word", value: word })
          }
        />
        <Codes
          isLoading={isLoading}
