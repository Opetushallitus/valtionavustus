  ...
  return (
    <div>
      <div className={questionClass} onClick={onQuestionClick}>
        {question.label}{" "}
        <span hidden={activeFilterCount === 0}>({activeFilterCount})</span>
      </div>
...
const FilterList = () => {
  ...
  const checkboxQuestions = FormUtil.findFieldsByFieldType(
    form.content,
    "checkboxButton"
  );
  const dropdownQuestions = FormUtil.findFieldsByFieldType(
    form.content,
    "dropdown"
  );
  const questions = radioQuestions
    .concat(checkboxQuestions)
    .concat(dropdownQuestions);
  ...
  return (
    <div hidden={!open} className="panel hakemus-filter-panel">
      <button
        ...
        data-test-id="rajaa-listaa-close"
      >
        x
      </button>
