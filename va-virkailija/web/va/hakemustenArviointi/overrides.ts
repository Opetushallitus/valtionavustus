...
export function mutateDefaultBudgetValuesForSelectedHakemusOverriddenAnswers(
  ...
  const budgetElement = FormUtil.findFieldByFieldType(hakuData.form.content, 'vaBudget')
  if (!budgetElement) {
    return
  }
...
export function mutatesDefaultBudgetValuesForSelectedHakemusSeurantaAnswers(
  ...
  initDefaultValues(selectedHakemus.arvio['seuranta-answers'], defaultValues, budgetElement, 'fi')
}
...
export function mutateDefaultTraineeDayValuesForSelectedHakemusOverriddenAnswers(
  ...
  const defaultFields: any = _.reduce(
    VaTraineeDayUtil.collectCalculatorSpecifications(hakuData.form.content, hakemusAnswers),
    (acc: any, field) => {
