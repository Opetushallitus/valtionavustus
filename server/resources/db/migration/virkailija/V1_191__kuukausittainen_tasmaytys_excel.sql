CREATE OR REPLACE VIEW v_tasmaytysraportti_excel_data AS
SELECT h.avustushaku               AS avustushaku
     , h.id                        AS hakemus
     ...
     , a.content -> 'name' -> 'fi' AS haun_nimi
     , c.code                      AS projektikoodi
FROM virkailija.payments p
       ...
       JOIN virkailija.va_code_values c ON c.id = h.project_id
       LEFT JOIN (SELECT id, version, elem ->> 'value' as value
                  FROM hakija.form_submissions f,
                       jsonb_array_elements(answers -> 'value') elem
                  WHERE f.version_closed IS NULL
                    AND elem ->> 'key' = 'bank-iban') AS iban
                 ON (h.form_submission_id = iban.id AND h.form_submission_version = iban.version)
       LEFT JOIN (SELECT id, version, elem ->> 'value' as value
                  FROM hakija.form_submissions f,
                       jsonb_array_elements(answers -> 'value') elem
                  WHERE f.version_closed IS NULL
                    AND elem ->> 'key' = 'radioButton-0') AS lkp
                 ON (h.form_submission_id = lkp.id AND h.form_submission_version = lkp.version)
