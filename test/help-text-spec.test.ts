...
describe("Tooltips", () => {
  ...
  describe("When haku has been created", () => {
    ...
    describe("And user navigates to päätös tab", () => {
      ...
      it("shows päätös help text", async () => {
        const decisionEditor = await page.waitForSelector(".decision-editor", {
          visible: true,
        });
        const mainHelpText = await page.evaluate(
          (element) => element.textContent,
          decisionEditor
        );
        expect(mainHelpText).toMatch(
  ...
  describe("When lukio haku has been created and published", () => {
    ...
    beforeAll(async () => {
      avustushakuID =
        await createValidCopyOfLukioEsimerkkihakuWithValintaperusteetAndReturnTheNewId(
          page
        );
