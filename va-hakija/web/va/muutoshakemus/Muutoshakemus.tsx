...
export const MuutoshakemusComponent = ({ query }: { query: Query }) => {
  ...
  const avustushakuId = query["avustushaku-id"]
    ? parseInt(query["avustushaku-id"])
    : 0;
  const [state, setState] = useState<MuutoshakemusProps>(initialState);
  const f = createFormikHook(userKey, lang);
  const existingNewMuutoshakemus = state.muutoshakemukset.find(
    (m) => m.status === "new"
  );
  const enableBudgetChange =
    state.hakemus?.talousarvio && state.hakemus.talousarvio.length > 1;
  ...
  useEffect(() => {
    const fetchProps = async () => {
      ...
      const [environment, avustushaku, hakemus, muutoshakemukset] =
        await Promise.all([
          environmentP,
          avustushakuP,
          hakemusP,
      ...
      const currentProjectEnd = getProjectEndMoment(
        avustushaku,
        muutoshakemukset
      );
  ...
  const existingMuutoshakemus = (
    ...
    const topic = `${t.muutoshakemus.title} ${moment(m["created-at"]).format(
      fiShortFormat
    )}`;
    const waitingForDecision =
      m.status === "new" ? ` - ${t.waitingForDecision}` : "";
    return (
      ...
      >
        ...
        <div className="existing-muutoshakemus__form">
          <MuutoshakemusValues
            currentTalousarvio={getTalousarvio(
              allMuutoshakemus,
      ...
      <ErrorBoundary>
        <MuutoshakemusSection className="muutoshakemus__top-form">
          ...
          {!existingNewMuutoshakemus && (
            <>
              <h2 className="muutoshakemus__sub-title">
                {t.applicationEdit.title}
              </h2>
              <div>
